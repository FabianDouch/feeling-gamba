...
as $$
  ...
  place_scored_predictions as (
    select
      prediction.*,
      coalesce(prediction.outcome_starter_count, prediction.predicted_starter_count, 0) as place_starter_count,
      case
        when prediction.country = 'HK' and coalesce(prediction.outcome_starter_count, prediction.predicted_starter_count, 0) >= 7 then 3
        when prediction.country = 'HK' and coalesce(prediction.outcome_starter_count, prediction.predicted_starter_count, 0) >= 4 then 2
        when coalesce(prediction.country, '') <> 'HK' and coalesce(prediction.outcome_starter_count, prediction.predicted_starter_count, 0) >= 8 then 3
        when coalesce(prediction.country, '') <> 'HK' and coalesce(prediction.outcome_starter_count, prediction.predicted_starter_count, 0) >= 5 then 2
        else 0
      end as place_payout_depth,
      coalesce(
        result.place_dividend,
        result.tote_place_dividend,
        case
          when day_entry.favourite_runner_id = prediction.outcome_runner_id
            or day_entry.favourite_runner_number = prediction.predicted_runner_number
            then day_entry.favourite_place_return
          else null
        end
      ) as outcome_place_dividend
    from filtered_predictions prediction
    left join public.race_results result
      on result.race_id = prediction.outcome_race_id
      and result.runner_id = prediction.outcome_runner_id
    left join public.race_day_entries day_entry
      on day_entry.race_id = prediction.outcome_race_id
  ),
  aggregate_values as (
    select
      ...
      count(*) filter (
        where outcome_status = 'settled'
          and place_payout_depth > 0
          and outcome_result_position between 1 and place_payout_depth
          and outcome_place_dividend is null
      )::int as missing_place_return_count,
      count(*) filter (
      ...
      coalesce(sum(1) filter (
        where outcome_status = 'settled'
          and place_payout_depth > 0
      ), 0)::numeric as total_place_stake,
      coalesce(sum(
        case
          when outcome_status = 'settled'
            and place_payout_depth > 0
            and outcome_result_position between 1 and place_payout_depth
            then coalesce(outcome_place_dividend, 0)
          when outcome_status = 'settled'
            and place_payout_depth > 0
            then 0
          else null
        end
      ), 0)::numeric as total_place_return,
  ...
  select
    ...
    round(total_place_stake, 2) as total_place_stake,
    round(total_place_return, 2) as total_place_return,
    round(total_place_return - total_place_stake, 2) as place_net_return,
    case when total_place_stake > 0 then round(total_place_return / total_place_stake, 3) else 0 end as place_average_return_per_dollar,
    case when total_place_stake > 0 then round(((total_place_return - total_place_stake) / total_place_stake) * 100, 2) else 0 end as place_roi_percentage,
