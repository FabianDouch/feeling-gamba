alter table public.multi_bet_recommendation_legs
  add column if not exists prediction_rank int;

update public.multi_bet_recommendation_legs
set prediction_rank = leg_index
where prediction_rank is null;

create index if not exists multi_bet_recommendation_legs_rank_idx
  on public.multi_bet_recommendation_legs (recommendation_id, prediction_rank);

drop function if exists public.get_multi_bet_recommendation_summary(text, date, date, text, text, text, text);
...
as $$
begin
  if p_max_leg_rank is null then
    return query
      with filtered as (
      ...
      summary as (
        select
          ...
          (
            ...
            + (count(*) filter (where outcome_status = 'race_not_found'))::int
  ...
  return query
    ...
    ranked_base as (
      select
        ...
        count(*) filter (where leg.outcome_status = 'settled' and leg.outcome_result_position = 1)::int as winning_leg_count,
        count(*) filter (where leg.outcome_status = 'pending')::int as pending_leg_count,
        count(*) filter (where leg.outcome_status in ('missing_result', 'race_not_found'))::int as missing_result_count,
      ...
      where coalesce(leg.prediction_rank, leg.leg_index) <= p_max_leg_rank
    ...
    ranked_recommendations as (
      select
        ...
        case
          ...
          when ranked_base.missing_runner_count > 0 then 'missing_runner'
          when ranked_base.missing_result_count > 0 then 'missing_result'
          else 'settled'
        end as outcome_status,
        case
          when ranked_base.settled_leg_count = ranked_base.leg_count
            and ranked_base.winning_leg_count = ranked_base.leg_count
    ...
    summary as (
      select
        ...
        count(*) filter (where outcome_status = 'settled')::int as settled_count,
        count(*) filter (where outcome_status = 'pending')::int as pending_count,
        coalesce(sum(missing_result_count), 0)::int as missing_result_count,
        coalesce(sum(missing_runner_count), 0)::int as missing_runner_count,
        count(*) filter (where outcome_status = 'settled')::numeric as total_stake,
        coalesce(sum(outcome_win_return) filter (where outcome_status = 'settled'), 0)::numeric as total_return,
        count(*) filter (where outcome_status = 'settled' and outcome_win_return > 0)::int as wins
...
drop function if exists public.get_multi_bet_recommendation_entries(text, date, date, text, text, text, text, int, int);
...
begin
  ...
  return query
    ...
    select
      ranked_rows.id,
      ranked_rows.prediction_model,
      ranked_rows.source_date,
      ranked_rows.predicted_at,
      ranked_rows.recommendation_type,
      ranked_rows.leg_count,
      ranked_rows.combined_fixed_win_price,
      ranked_rows.average_cash_score,
      ranked_rows.outcome_status,
      ranked_rows.outcome_win_return,
      ranked_rows.outcome_settled_leg_count,
      ranked_rows.outcome_winning_leg_count,
      ranked_rows.outcome_missing_result_count,
      ranked_rows.outcome_missing_runner_count,
      coalesce(jsonb_agg(
        jsonb_build_object(
          'legIndex', leg.leg_index,
          'predictionRank', coalesce(leg.prediction_rank, leg.leg_index),
        ...
        order by coalesce(leg.prediction_rank, leg.leg_index)
      ) filter (where leg.id is not null), '[]'::jsonb) as legs,
      ranked_rows.total_count
    from ranked_rows
    join public.multi_bet_recommendation_legs leg
      on leg.recommendation_id = ranked_rows.id
    ...
    order by
      case
        when ranked_rows.outcome_status = 'settled' and ranked_rows.outcome_win_return > 0 then 0
        when ranked_rows.outcome_status = 'settled' then 1
        when ranked_rows.outcome_status = 'pending' then 2
        else 3
      end,
      ranked_rows.source_date desc,
      ranked_rows.predicted_at desc
    limit greatest(p_limit, 0)
    offset greatest(p_offset, 0);
end;
$$;
