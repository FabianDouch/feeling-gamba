create or replace function public.is_multi_bet_leg_successful(
  p_prediction_model text,
  p_outcome_status text,
  p_outcome_result_position int,
  p_outcome_win_return numeric,
  p_raw jsonb
)
returns boolean
language sql
...
as $$
  select
    p_outcome_status = 'settled'
    and (
      case
        when p_prediction_model = 'multi_place_percentage_v1' then
          coalesce(p_outcome_win_return, 0) > 0
          or (
            p_outcome_result_position is not null
            and p_outcome_result_position <= coalesce(
              nullif(p_raw #>> '{placingCandidate,placePayoutDepth}', '')::int,
              nullif(p_raw ->> 'placePayoutDepth', '')::int,
              0
            )
          )
        else p_outcome_result_position = 1
      end
    );
$$;
    ...
    select
      ...
      case
        when ranked_base.row_settled_leg_count = ranked_base.row_leg_count
          ...
          then case
            when p_prediction_model = 'multi_place_percentage_v1' then 1
    ...
    select
      ...
      case
        when ranked_base.outcome_settled_leg_count = ranked_base.leg_count
          ...
          then case
            when ranked_base.prediction_model = 'multi_place_percentage_v1' then 1
