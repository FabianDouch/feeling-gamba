alter table public.multi_bet_recommendations
  add column if not exists combined_fixed_place_price numeric(12, 2);

alter table public.multi_bet_recommendation_legs
  add column if not exists predicted_fixed_place_price numeric(12, 2);

comment on column public.multi_bet_recommendations.combined_fixed_place_price is
  'Product of stored fixed-place leg prices for place-percentage multi recommendations.';

comment on column public.multi_bet_recommendation_legs.predicted_fixed_place_price is
  'Source-observed fixed-place decimal price captured when the multi leg was recommended.';
...
as $$
  ...
  ranked_base as (
    ...
    group by recommendation.id, recommendation.prediction_model, recommendation.source_date
    ...
    select
      ...
      case
        when ranked_base.row_settled_leg_count = ranked_base.row_leg_count
          ...
          then case
            when ranked_base.prediction_model = 'multi_place_percentage_v1' then ranked_base.row_combined_fixed_place_price
            else ranked_base.row_combined_fixed_win_price
          end
        else 0
...
drop function if exists public.get_multi_bet_recommendation_entries(text, date, date, text, text, text, text, int, int, int);

create function public.get_multi_bet_recommendation_entries(
  p_prediction_model text,
  p_from_date date,
  p_to_date date,
  p_country text default null,
...
as $$
  ...
  parent_rows as (
    select
      ...
      recommendation.combined_fixed_place_price,
  ...
  ranked_rows as (
    select
      ...
      ranked_base.combined_fixed_place_price,
      ...
      case
        when ranked_base.outcome_settled_leg_count = ranked_base.leg_count
          ...
          then case
            when ranked_base.prediction_model = 'multi_place_percentage_v1' then ranked_base.combined_fixed_place_price
            else ranked_base.combined_fixed_win_price
          end
        else 0
