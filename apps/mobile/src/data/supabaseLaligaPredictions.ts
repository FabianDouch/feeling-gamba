...
type LaligaSinglePredictionRow = {
  ...
  prediction_model: LaligaSinglePredictionModelKey;
...
/**
 * Reads the latest generated La Liga single predictions for one model.
 */
export async function fetchCurrentLaligaSinglePredictions(
  modelKey: LaligaSinglePredictionModelKey,
): Promise<LaligaSinglePredictionsResult> {
  const latestRows = await supabaseSelectAll<{ source_date: string }>("laliga_single_predictions", {
    limit: "1",
  ...
  const rows = await supabaseSelectAll<LaligaSinglePredictionRow>("laliga_single_predictions", {
    order: "prediction_rank.asc,advertised_start_at.asc",
    prediction_model: `eq.${modelKey}`,
    select: LALIGA_SINGLE_PREDICTION_SELECT,
    source_date: `eq.${sourceDate}`,
  });

  return {
    generatedAt: rows[0]?.predicted_at ?? null,
    predictions: rows.map(mapLaligaSinglePrediction),
    sourceDate,
    totalCount: rows.length,
  };
}

const LALIGA_SINGLE_PREDICTION_SELECT = [
  "advertised_start_at",
  "bucket_sample_size",
  "id",
  "lineup_status",
...
/**
 * Maps one stored La Liga prediction row to the card model used in Predictions.
 */
function mapLaligaSinglePrediction(row: LaligaSinglePredictionRow): LaligaSinglePredictionItem {
  ...
  return {
    ...
    id: row.id,
    matchLabel: row.match_label ?? "La Liga match",
    meta: [
