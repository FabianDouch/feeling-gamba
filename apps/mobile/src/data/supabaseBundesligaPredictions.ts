...
type BundesligaSinglePredictionRow = {
  ...
  prediction_model: BundesligaSinglePredictionModelKey;
...
/**
 * Reads the latest generated Bundesliga single predictions for one model.
 */
export async function fetchCurrentBundesligaSinglePredictions(
  modelKey: BundesligaSinglePredictionModelKey,
): Promise<BundesligaSinglePredictionsResult> {
  const latestRows = await supabaseSelectAll<{ source_date: string }>("bundesliga_single_predictions", {
    limit: "1",
  ...
  const rows = await supabaseSelectAll<BundesligaSinglePredictionRow>("bundesliga_single_predictions", {
    order: "prediction_rank.asc,advertised_start_at.asc",
    prediction_model: `eq.${modelKey}`,
    select: BUNDESLIGA_SINGLE_PREDICTION_SELECT,
    source_date: `eq.${sourceDate}`,
  });

  return {
    generatedAt: rows[0]?.predicted_at ?? null,
    predictions: rows.map(mapBundesligaSinglePrediction),
    sourceDate,
    totalCount: rows.length,
  };
}

const BUNDESLIGA_SINGLE_PREDICTION_SELECT = [
  "advertised_start_at",
  "bucket_sample_size",
  "id",
  "lineup_status",
...
/**
 * Maps one stored Bundesliga prediction row to the card model used in Predictions.
 */
function mapBundesligaSinglePrediction(row: BundesligaSinglePredictionRow): BundesligaSinglePredictionItem {
  ...
  return {
    ...
    id: row.id,
    matchLabel: row.match_label ?? "Bundesliga match",
    meta: [
