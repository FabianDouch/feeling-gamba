...
type MlsSinglePredictionRow = {
  ...
  prediction_model: MlsSinglePredictionModelKey;
...
/**
 * Reads the latest generated MLS single predictions for one model.
 */
export async function fetchCurrentMlsSinglePredictions(
  modelKey: MlsSinglePredictionModelKey,
): Promise<MlsSinglePredictionsResult> {
  const latestRows = await supabaseSelectAll<{ source_date: string }>("mls_single_predictions", {
    limit: "1",
  ...
  const rows = await supabaseSelectAll<MlsSinglePredictionRow>("mls_single_predictions", {
    order: "prediction_rank.asc,advertised_start_at.asc",
    prediction_model: `eq.${modelKey}`,
    select: MLS_SINGLE_PREDICTION_SELECT,
    source_date: `eq.${sourceDate}`,
  });

  return {
    generatedAt: rows[0]?.predicted_at ?? null,
    predictions: rows.map(mapMlsSinglePrediction),
    sourceDate,
    totalCount: rows.length,
  };
}

const MLS_SINGLE_PREDICTION_SELECT = [
  "advertised_start_at",
  "bucket_sample_size",
  "id",
  "lineup_status",
...
/**
 * Maps one stored MLS prediction row to the card model used in Predictions.
 */
function mapMlsSinglePrediction(row: MlsSinglePredictionRow): MlsSinglePredictionItem {
  ...
  return {
    ...
    id: row.id,
    matchLabel: row.match_label ?? "MLS match",
    meta: [
