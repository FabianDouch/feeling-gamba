...
type SerieaSinglePredictionRow = {
  ...
  prediction_model: SerieaSinglePredictionModelKey;
...
/**
 * Reads the latest generated Serie A single predictions for one model.
 */
export async function fetchCurrentSerieaSinglePredictions(
  modelKey: SerieaSinglePredictionModelKey,
): Promise<SerieaSinglePredictionsResult> {
  const latestRows = await supabaseSelectAll<{ source_date: string }>("seriea_single_predictions", {
    limit: "1",
  ...
  const rows = await supabaseSelectAll<SerieaSinglePredictionRow>("seriea_single_predictions", {
    order: "prediction_rank.asc,advertised_start_at.asc",
    prediction_model: `eq.${modelKey}`,
    select: SERIEA_SINGLE_PREDICTION_SELECT,
    source_date: `eq.${sourceDate}`,
  });

  return {
    generatedAt: rows[0]?.predicted_at ?? null,
    predictions: rows.map(mapSerieaSinglePrediction),
    sourceDate,
    totalCount: rows.length,
  };
}

const SERIEA_SINGLE_PREDICTION_SELECT = [
  "advertised_start_at",
  "bucket_sample_size",
  "id",
  "lineup_status",
...
/**
 * Maps one stored Serie A prediction row to the card model used in Predictions.
 */
function mapSerieaSinglePrediction(row: SerieaSinglePredictionRow): SerieaSinglePredictionItem {
  ...
  return {
    ...
    id: row.id,
    matchLabel: row.match_label ?? "Serie A match",
    meta: [
