...
type Ligue1SinglePredictionRow = {
  ...
  prediction_model: Ligue1SinglePredictionModelKey;
...
/**
 * Reads the latest generated Ligue 1 single predictions for one model.
 */
export async function fetchCurrentLigue1SinglePredictions(
  modelKey: Ligue1SinglePredictionModelKey,
): Promise<Ligue1SinglePredictionsResult> {
  const latestRows = await supabaseSelectAll<{ source_date: string }>("ligue1_single_predictions", {
    limit: "1",
  ...
  const rows = await supabaseSelectAll<Ligue1SinglePredictionRow>("ligue1_single_predictions", {
    order: "prediction_rank.asc,advertised_start_at.asc",
    prediction_model: `eq.${modelKey}`,
    select: LIGUE1_SINGLE_PREDICTION_SELECT,
    source_date: `eq.${sourceDate}`,
  });

  return {
    generatedAt: rows[0]?.predicted_at ?? null,
    predictions: rows.map(mapLigue1SinglePrediction),
    sourceDate,
    totalCount: rows.length,
  };
}

const LIGUE1_SINGLE_PREDICTION_SELECT = [
  "advertised_start_at",
  "bucket_sample_size",
  "id",
  "lineup_status",
...
/**
 * Maps one stored Ligue 1 prediction row to the card model used in Predictions.
 */
function mapLigue1SinglePrediction(row: Ligue1SinglePredictionRow): Ligue1SinglePredictionItem {
  ...
  return {
    ...
    id: row.id,
    matchLabel: row.match_label ?? "Ligue 1 match",
    meta: [
