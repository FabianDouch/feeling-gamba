...
const NATIONSLEAGUE_CATEGORY = "SOCCER";
const NATIONSLEAGUE_COMPETITION_SLUG = "uefa-nations-league";
const NATIONSLEAGUE_FIXED_WIN_MARKET_NAME = "match result";
const TAB_SOURCE = {
  endpoint: "https://api.tab.co.nz/graphql",
  label: "UEFA Nations League market source",
  source: "tab",
};
const TEAM_NAME_ALIASES = NATIONSLEAGUE_TEAM_ALIASES;

const NATIONSLEAGUE_COMPETITION_QUERY = `
...
/**
 * Parses the UEFA Nations League fixed-win snapshot options for the current market source.
 */
function parseArgs(argv) {
  const options = {
    batchSize: DEFAULT_BATCH_SIZE,
    competitionSlug: NATIONSLEAGUE_COMPETITION_SLUG,
    dryRun: false,
...
/**
 * Builds browser-like headers for the UEFA Nations League market source request.
 */
...
function findFixedWinMarket(event) {
  return (event.markets?.nodes ?? []).find((market) =>
    normalizeName(market?.name) === NATIONSLEAGUE_FIXED_WIN_MARKET_NAME);
}
...
function findDrawEntrant(market) {
  return (market.entrants?.nodes ?? []).find((entrant) =>
    normalizeName(entrant?.name) === "draw" && entrant?.isSuspended !== true);
}
...
function matchExistingNationsleagueMatch(snapshot, matches) {
...
function mapSnapshot(source, event, generatedAt, officialMatches) {
  ...
  const snapshot = {
    ...
    draw_fixed_win_price: drawPrice,
  ...
  const matchedMatch = matchExistingNationsleagueMatch(snapshot, officialMatches);

  return {
    ...snapshot,
    matched_nationsleague_match_id: matchedMatch?.id ?? null,
  };
}

/**
 * Fetches current UEFA Nations League fixed-win snapshots from the selected public market source.
 */
async function fetchSnapshots(options, officialMatches) {
  ...
  for (const source of [TAB_SOURCE]) {
    const response = await graphql(source, "SportingCompetitionScreen", NATIONSLEAGUE_COMPETITION_QUERY, {
      category: NATIONSLEAGUE_CATEGORY,
...
async function fetchOfficialMatchesForWindow(config, rows, batchSize) {
  ...
  try {
    return await supabase.request("nationsleague_matches", {
      search: {
    ...
    if (isMissingNationsleagueSchemaError(error)) {
      return [];
    }
...
function isMissingNationsleagueSchemaError(error) {
  return error instanceof Error
    && error.message.includes("PGRST205")
    && error.message.includes("nationsleague_matches");
}
...
async function rematchRows(config, rows, batchSize) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_nationsleague_match_id: matchExistingNationsleagueMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("nationsleague_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_nationsleague_match_id).length,
    nationsleagueMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_nationsleague_match_id).length,
    nationsleagueMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
  ...
  if (options.dryRun) {
    console.log(JSON.stringify({
      ...
      sample: matchedRows.slice(0, 5).map((row) => ({
        ...
        draw: {
          price: row.draw_fixed_win_price,
        },
        favourite: {
        ...
        matchedNationsleagueMatchId: row.matched_nationsleague_match_id,
        source: row.source,
