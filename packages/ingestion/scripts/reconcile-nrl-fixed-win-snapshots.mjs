...
/**
 * Parses NRL fixed-win snapshot reconciliation options.
 */
function parseArgs(argv) {
  ...
  if (!["all", "betcha", "tab"].includes(options.source)) {
    throw new Error("--source must be all, betcha, or tab.");
  }
...
/**
 * Minimal Supabase REST client for NRL fixed-win reconciliation.
 */
...
/**
 * Confirms an official NRL match has final scores usable for settlement.
 */
...
/**
 * Derives the winning side from official final scores, leaving draws unresolved.
 */
...
/**
 * Classifies one snapshot against its matched official NRL result state.
 */
function mapOutcome(snapshot, match) {
  if (!snapshot.matched_nrl_match_id) {
    return {
...
async function readSnapshots(supabase, options) {
  ...
  const rows = await supabase.request("nrl_market_snapshots", {
    search,
  });
...
/**
 * Loads official NRL matches referenced by the selected snapshots.
 */
async function readMatches(supabase, snapshots) {
  const matchIds = Array.from(new Set(
    snapshots
      .map((snapshot) => snapshot.matched_nrl_match_id)
      .filter(Boolean),
  ));
  ...
  const rows = await supabase.request("nrl_matches", {
    search: {
      id: `in.(${matchIds.join(",")})`,
      select: [
        "id",
...
/**
 * Builds fixed-win outcome rows from source snapshots and official NRL results.
 */
function reconcileSnapshots(snapshots, matchesById) {
  ...
  for (const snapshot of snapshots) {
    const match = snapshot.matched_nrl_match_id
      ? matchesById.get(snapshot.matched_nrl_match_id)
      : null;
...
async function writeRows(supabase, rows) {
  ...
  return {
    nrlFixedWinSnapshotResults: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(snapshots, matchesById, reconciliation) {
  return {
    matchedSnapshots: snapshots.filter((snapshot) => snapshot.matched_nrl_match_id).length,
