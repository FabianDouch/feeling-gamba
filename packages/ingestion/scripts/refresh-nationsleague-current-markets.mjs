...
const DEFAULT_COMPETITION_SLUG = "uefa-nations-league";
...
/**
 * Builds the shared Supabase/write flags used by the child UEFA Nations League workers.
 */
...
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_nationsleague_fixed_win_markets", "refresh-nationsleague-market-snapshots-from-tab.mjs", [
  ...
  if (!options.skipGoalScorers || !options.skipReconcile || !options.skipPredictions) {
    commands.push(buildCommand("refresh_priced_nationsleague_official_matches", "refresh-nationsleague-results-from-official.mjs", [
      "--include-fixtures",
      "--priced-only",
      ...writeFlags,
    ]));
...
/**
 * Runs the UEFA Nations League current-market capture and derived read-model refresh pipeline.
 */
async function main() {
