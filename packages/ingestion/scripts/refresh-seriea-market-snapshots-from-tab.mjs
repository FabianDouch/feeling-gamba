...
const SERIEA_CATEGORY = "SOCCER";
const SERIEA_COMPETITION_SLUG = "italian-serie-a";
const SERIEA_FIXED_WIN_MARKET_NAME = "match result";
const TAB_SOURCE = {
...
/**
 * Parses the Serie A fixed-win snapshot options for the current market source.
 */
function parseArgs(argv) {
  const options = {
    batchSize: DEFAULT_BATCH_SIZE,
    competitionSlug: SERIEA_COMPETITION_SLUG,
    dryRun: false,
...
/**
 * Builds browser-like headers for the Serie A market source request.
 */
...
function findFixedWinMarket(event) {
  return (event.markets?.nodes ?? []).find((market) =>
    normalizeName(market?.name) === SERIEA_FIXED_WIN_MARKET_NAME);
}
...
function matchExistingSerieaMatch(snapshot, matches) {
...
function mapSnapshot(source, event, generatedAt, officialMatches) {
  ...
  const matchedMatch = matchExistingSerieaMatch(snapshot, officialMatches);

  return {
    ...snapshot,
    matched_seriea_match_id: matchedMatch?.id ?? null,
  };
}

/**
 * Fetches current Serie A fixed-win snapshots from the selected public market source.
 */
async function fetchSnapshots(options, officialMatches) {
  ...
  for (const source of [TAB_SOURCE]) {
    const response = await graphql(source, "SportingCompetitionScreen", SERIEA_COMPETITION_QUERY, {
      category: SERIEA_CATEGORY,
...
/**
 * Minimal Supabase REST client for Serie A market snapshot reads and writes.
 */
...
function isMissingSerieaSchemaError(error) {
  return error instanceof Error
    && error.message.includes("PGRST205")
    && error.message.includes("seriea_matches");
}
...
async function rematchRows(config, rows, batchSize) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_seriea_match_id: matchExistingSerieaMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("seriea_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_seriea_match_id).length,
    serieaMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_seriea_match_id).length,
    serieaMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
  ...
  if (options.dryRun) {
    console.log(JSON.stringify({
      ...
      sample: matchedRows.slice(0, 5).map((row) => ({
        ...
        matchedSerieaMatchId: row.matched_seriea_match_id,
        source: row.source,
