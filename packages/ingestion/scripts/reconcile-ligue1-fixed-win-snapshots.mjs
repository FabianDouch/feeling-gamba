...
function mapOutcome(snapshot, match) {
  if (!snapshot.matched_ligue1_match_id) {
    return {
...
async function readSnapshots(supabase, options) {
  ...
  const rows = await supabase.request("ligue1_market_snapshots", {
    search,
  });
...
function reconcileSnapshots(snapshots, matchesById) {
  ...
  for (const snapshot of snapshots) {
    const match = snapshot.matched_ligue1_match_id
...
async function writeSnapshotMatchUpdates(supabase, updates) {
  for (const update of updates) {
    await supabase.request("ligue1_market_snapshots", {
      body: {
        matched_ligue1_match_id: update.matched_ligue1_match_id,
      },
      method: "PATCH",
...
async function writeRows(supabase, rows, snapshotMatchUpdates) {
  ...
  await supabase.upsert(
    "ligue1_fixed_win_snapshot_results",
    rows,
    "source_snapshot_key",
  );

  return {
    ligue1FixedWinSnapshotResults: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(snapshots, matchesById, reconciliation) {
  return {
    matchedSnapshots: snapshots.filter((snapshot) => snapshot.matched_ligue1_match_id).length,
