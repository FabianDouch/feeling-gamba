...
function mapOutcome(snapshot, match) {
  if (!snapshot.matched_eflcup_match_id) {
    return {
...
async function readSnapshots(supabase, options) {
  ...
  const rows = await supabase.request("eflcup_market_snapshots", {
    search,
  });
...
function resolveSnapshotMatches(snapshots, officialMatches) {
  ...
  const resolvedSnapshots = snapshots.map((snapshot) => {
    if (snapshot.matched_eflcup_match_id && matchesById.has(snapshot.matched_eflcup_match_id)) {
      return snapshot;
    }

    const match = matchExistingEflcupMatch(snapshot, officialMatches);
...
function reconcileSnapshots(snapshots, matchesById) {
  ...
  for (const snapshot of snapshots) {
    const match = snapshot.matched_eflcup_match_id
      ? matchesById.get(snapshot.matched_eflcup_match_id)
      : null;
...
async function writeSnapshotMatchUpdates(supabase, updates) {
  for (const update of updates) {
    await supabase.request("eflcup_market_snapshots", {
      body: {
        matched_eflcup_match_id: update.matched_eflcup_match_id,
      },
      method: "PATCH",
...
async function writeRows(supabase, rows, snapshotMatchUpdates) {
  ...
  await supabase.upsert(
    "eflcup_fixed_win_snapshot_results",
    rows,
    "source_snapshot_key",
  );

  return {
    eflcupFixedWinSnapshotResults: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(snapshots, matchesById, reconciliation) {
  return {
    matchedSnapshots: snapshots.filter((snapshot) => snapshot.matched_eflcup_match_id).length,
