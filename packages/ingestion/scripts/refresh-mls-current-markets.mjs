...
const DEFAULT_COMPETITION_SLUG = "major-league-soccer";

/**
 * Parses the MLS current-market refresh orchestration options.
 */
...
/**
 * Builds the shared Supabase/write flags used by the child MLS workers.
 */
...
/**
 * Builds the ordered MLS current-market refresh command list.
 */
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_mls_fixed_win_markets", "refresh-mls-market-snapshots-from-tab.mjs", [
  ...
  if (!options.skipGoalScorers || !options.skipReconcile || !options.skipPredictions) {
    commands.push(buildCommand("refresh_priced_mls_official_matches", "refresh-football-league-results-from-json.mjs", [
      "--league=mls",
      "--include-fixtures",
      "--priced-only",
      ...writeFlags,
  ...
  if (!options.skipGoalScorers) {
    commands.push(buildCommand("capture_mls_goal_scorer_markets", "refresh-mls-goal-scorer-market-snapshots-from-tab.mjs", [
      `--competition-slug=${options.competitionSlug}`,
  ...
  if (!options.skipReconcile) {
    commands.push(buildCommand("reconcile_mls_fixed_win", "reconcile-mls-fixed-win-snapshots.mjs", writeFlags));
  }

  if (!options.skipSameGameMultis) {
    commands.push(buildCommand("rebuild_mls_same_game_multis", "rebuild-mls-same-game-multis.mjs", writeFlags));
  }

  if (!options.skipInsights) {
    commands.push(buildCommand("rebuild_mls_insights", "rebuild-mls-insight-aggregates.mjs", writeFlags));
  }

  if (!options.skipPredictions) {
    commands.push(buildCommand("generate_mls_single_predictions", "generate-mls-single-predictions.mjs", writeFlags));
  }
...
/**
 * Runs the MLS current-market capture and derived read-model refresh pipeline.
 */
async function main() {
