...
const BUNDESLIGA_FIXED_WIN_MARKET_NAME = "match result";
const TAB_SOURCE = {
...
/**
 * Parses the Bundesliga fixed-win snapshot options for the current market source.
 */
function parseArgs(argv) {
  const options = {
    batchSize: DEFAULT_BATCH_SIZE,
    competitionSlug: BUNDESLIGA_COMPETITION_SLUG,
    dryRun: false,
...
/**
 * Builds browser-like headers for the Bundesliga market source request.
 */
...
function findFixedWinMarket(event) {
  return (event.markets?.nodes ?? []).find((market) =>
    normalizeName(market?.name) === BUNDESLIGA_FIXED_WIN_MARKET_NAME);
}
...
function mapSnapshot(source, event, generatedAt, officialMatches) {
  ...
  const matchedMatch = matchExistingBundesligaMatch(snapshot, officialMatches);

  return {
    ...snapshot,
    matched_bundesliga_match_id: matchedMatch?.id ?? null,
  };
}

/**
 * Fetches current Bundesliga fixed-win snapshots from the selected public market source.
 */
async function fetchSnapshots(options, officialMatches) {
  ...
  for (const source of [TAB_SOURCE]) {
    const response = await graphql(source, "SportingCompetitionScreen", BUNDESLIGA_COMPETITION_QUERY, {
      category: BUNDESLIGA_CATEGORY,
...
/**
 * Minimal Supabase REST client for Bundesliga market snapshot reads and writes.
 */
...
async function fetchOfficialMatchesForWindow(config, rows, batchSize) {
  ...
  try {
    ...
    if (isMissingBundesligaSchemaError(error)) {
      return [];
    }
...
function isMissingBundesligaSchemaError(error) {
  return error instanceof Error
    && error.message.includes("PGRST205")
    && error.message.includes("bundesliga_matches");
}
...
async function rematchRows(config, rows, batchSize) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_bundesliga_match_id: matchExistingBundesligaMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("bundesliga_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_bundesliga_match_id).length,
    bundesligaMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_bundesliga_match_id).length,
    bundesligaMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
  ...
  if (options.dryRun) {
    console.log(JSON.stringify({
      ...
      sample: matchedRows.slice(0, 5).map((row) => ({
        ...
        matchedBundesligaMatchId: row.matched_bundesliga_match_id,
        source: row.source,
