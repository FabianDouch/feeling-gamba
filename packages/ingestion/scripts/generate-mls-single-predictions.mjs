...
const FIXED_WIN_MODEL = "mls_fixed_win_percentage_single_v1";
const GOAL_SCORER_MODEL = "mls_goal_scorer_percentage_single_v1";
...
const MLS_SINGLE_PREDICTION_COLUMNS = [
  "advertised_start_at",
  "away_team_name",
  "bucket_sample_size",
  "home_team_name",
...
/**
 * Applies explicit TAB-vs-MLS club aliases after basic name normalization.
 */
...
function buildFixedWinPredictions({ matchesById, predictedAt, snapshots, sourceDate, teamRecords }) {
  ...
  for (const snapshot of getLatestSnapshots(snapshots)) {
    if (!snapshot.favourite_team_name || !snapshot.matched_mls_match_id) {
      continue;
    }

    const match = matchesById.get(snapshot.matched_mls_match_id);
    ...
    rows.push({
      ...
      matched_mls_match_id: snapshot.matched_mls_match_id,
...
function normalizeRowForWrite(row) {
  return Object.fromEntries(MLS_SINGLE_PREDICTION_COLUMNS.map((column) => [
    column,
    row[column] ?? null,
  ]));
}
...
async function readSourceRows(supabase) {
  const [snapshots, matches, aggregates] = await Promise.all([
    supabase.selectAll("mls_market_snapshots", {
    ...
    supabase.selectAll("mls_insight_aggregates", {
...
async function writeRows(supabase, rows, sourceDate) {
  await supabase.request("mls_single_predictions", {
    expectJson: false,
    method: "DELETE",
  ...
  await supabase.upsert(
    "mls_single_predictions",
    rows.map(normalizeRowForWrite),
    "source_prediction_key",
  );

  return {
    mlsSinglePredictions: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(sourceRows, fixedWinRows, goalScorerRows) {
  return {
    ...
    sourceMlsInsightAggregates: sourceRows.aggregates.length,
    sourceMlsMatches: sourceRows.matches.length,
