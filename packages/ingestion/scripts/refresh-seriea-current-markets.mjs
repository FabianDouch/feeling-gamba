...
const DEFAULT_COMPETITION_SLUG = "italian-serie-a";

/**
 * Parses the Serie A current-market refresh orchestration options.
 */
...
/**
 * Builds the shared Supabase/write flags used by the child Serie A workers.
 */
...
/**
 * Builds the ordered Serie A current-market refresh command list.
 */
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_seriea_fixed_win_markets", "refresh-seriea-market-snapshots-from-tab.mjs", [
  ...
  if (!options.skipGoalScorers || !options.skipReconcile || !options.skipPredictions) {
    commands.push(buildCommand("refresh_priced_seriea_official_matches", "refresh-football-league-results-from-json.mjs", [
      "--league=seriea",
      "--include-fixtures",
      "--priced-only",
      ...writeFlags,
  ...
  if (!options.skipGoalScorers) {
    commands.push(buildCommand("capture_seriea_goal_scorer_markets", "refresh-seriea-goal-scorer-market-snapshots-from-tab.mjs", [
      `--competition-slug=${options.competitionSlug}`,
  ...
  if (!options.skipReconcile) {
    commands.push(buildCommand("reconcile_seriea_fixed_win", "reconcile-seriea-fixed-win-snapshots.mjs", writeFlags));
  }

  if (!options.skipSameGameMultis) {
    commands.push(buildCommand("rebuild_seriea_same_game_multis", "rebuild-seriea-same-game-multis.mjs", writeFlags));
  }
  ...
  if (!options.skipPredictions) {
    commands.push(buildCommand("generate_seriea_single_predictions", "generate-seriea-single-predictions.mjs", writeFlags));
  }
...
/**
 * Runs the Serie A current-market capture and derived read-model refresh pipeline.
 */
async function main() {
