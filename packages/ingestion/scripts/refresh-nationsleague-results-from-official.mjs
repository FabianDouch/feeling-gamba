...
import { getNationsleagueSeasonYear, getNationsleagueRegularScore, NATIONSLEAGUE_TEAM_ALIASES } from "./lib/nationsleague.mjs";

const DOT_ENV_FILES = [".env.local", ".env"];
const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(SCRIPT_DIR, "../../..");
const DEFAULT_BATCH_SIZE = 300;
const DEFAULT_COMPETITION_ID = 2014;
...
/**
 * Maps a calendar date to UEFA's Nations League seasonYear convention.
 */
function getDefaultUefaSeasonYear(date) {
  return getNationsleagueSeasonYear(date);
}
...
/**
 * Fetches the UEFA Nations League match list for a season.
 */
...
function normalizeTeamName(value) {
  ...
  return NATIONSLEAGUE_TEAM_ALIASES.get(name) ?? name;
}
...
/** Reads only regulation-time scores for the three-way Match Result market. */
function getScore(match, side) {
  return getNationsleagueRegularScore(match, side);
}

/** A completed match without explicit regulation scores must remain unsettled. */
function getMatchStatus(match) {
  ...
  if (status.includes("FINISH") || status === "FT" || status === "PLAYED") {
    return getScore(match, "home") !== null && getScore(match, "away") !== null ? "settled" : "pending";
  }
...
/** Uses the matchday sequence rather than UEFA's internal round identifier. */
function getRoundNumber(match) {
  const value = match?.matchday?.sequenceNumber
...
async function readPricedSnapshots(supabase) {
  try {
    return await supabase.selectAll("nationsleague_market_snapshots", {
...
function isMissingNationsleagueSchemaError(error) {
  return error instanceof Error
    ...
    && error.message.includes("nationsleague_");
}
...
/**
 * Maps a UEFA match row into the app's UEFA Nations League match table.
 */
export function mapMatch(match, options) {
  const homeTeam = match.homeTeam ?? match.home_team ?? {};
  const awayTeam = match.awayTeam ?? match.away_team ?? {};
  ...
  return {
    ...
    source_url: sourceMatchId ? `https://www.uefa.com/uefanationsleague/match/${sourceMatchId}/` : null,
...
async function writeRows(supabase, rows) {
  await supabase.upsert("nationsleague_teams", rows.teams, "source,source_team_id");
  await supabase.upsert("nationsleague_players", rows.players, "source,source_player_id");
  await supabase.upsert("nationsleague_matches", rows.matches, "source,source_match_id");
  await supabase.upsert("nationsleague_player_match_appearances", rows.appearances, "source_appearance_key");
  await supabase.upsert("nationsleague_goal_scorers", rows.goalScorers, "source_goal_key");

  return {
    ok: true,
    skipped: false,
    nationsleagueAppearances: rows.appearances.length,
    nationsleagueGoalScorers: rows.goalScorers.length,
    nationsleagueMatches: rows.matches.length,
    nationsleaguePlayers: rows.players.length,
    nationsleagueTeams: rows.teams.length,
  };
}
