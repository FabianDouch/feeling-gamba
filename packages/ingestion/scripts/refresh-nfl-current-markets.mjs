...
/**
 * Parses the NFL current-market refresh orchestration options.
 */
function parseArgs(argv) {
  ...
  if (!options.skipFixturePreload && (options.round !== null || options.fromRound !== null) && !isPositiveInteger(options.season)) {
    throw new Error("Pass --season=YYYY when preloading official NFL fixtures.");
  }
...
/**
 * Builds the ordered NFL current-market refresh command list.
 */
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_nfl_fixed_win_markets", "refresh-nfl-market-snapshots-from-tab.mjs", [
      ...marketFlags,
  ...
  if (!options.skipReconcile) {
    commands.push(buildCommand("reconcile_nfl_fixed_win", "reconcile-nfl-fixed-win-snapshots.mjs", writeFlags));
  }

  if (!options.skipInsights) {
    commands.push(buildCommand("rebuild_nfl_insights", "rebuild-nfl-insight-aggregates.mjs", writeFlags));
  }
...
/**
 * Runs the complete current NFL market capture and derived-read-model refresh.
 */
async function main() {
