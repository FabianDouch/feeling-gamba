...
const LALIGA_CATEGORY = "SOCCER";
const LALIGA_COMPETITION_SLUG = "spanish-la-liga";
const LALIGA_FIXED_WIN_MARKET_NAME = "match result";
const LALIGA_GOAL_SCORER_MARKET_NAME = "anytime goalscorer";
const TAB_SOURCE = {
  endpoint: "https://api.tab.co.nz/graphql",
  label: "LaLiga market source",
  source: "tab",
};
...
const LALIGA_GOAL_SCORER_SNAPSHOT_QUERY = `
  query LaligaGoalScorerMarketSnapshot(
...
/**
 * Parses LaLiga player goal-scorer snapshot ingestion options.
 */
function parseArgs(argv) {
  const options = {
    batchSize: DEFAULT_BATCH_SIZE,
    competitionSlug: LALIGA_COMPETITION_SLUG,
    dryRun: false,
...
/**
 * Builds browser-like headers for the current LaLiga market source request.
 */
...
function findFixedWinMarket(event) {
  return (event.markets?.nodes ?? []).find((market) =>
    normalizeName(market?.name) === LALIGA_FIXED_WIN_MARKET_NAME);
}

/**
 * Finds all exact anytime goal-scorer markets on one event payload.
 */
function findGoalScorerMarkets(event) {
  return (event.markets?.nodes ?? []).filter((market) =>
    normalizeName(market?.name) === LALIGA_GOAL_SCORER_MARKET_NAME);
}
...
/**
 * Checks whether the market event teams match an official LaLiga fixture.
 */
...
/**
 * Matches a current market event to one official LaLiga fixture shell/result.
 */
function matchExistingLaligaMatch(eventContext, matches) {
...
function isPlayerSelection(playerName) {
  return normalizeName(playerName) !== "no goalscorer";
}
...
/**
 * Matches a market player entrant to one official LaLiga match appearance.
 */
...
/**
 * Maps exact anytime goal-scorer market entrants into snapshot rows.
 */
function mapGoalScorerRows({ appearancesByMatch, event, eventContext, generatedAt, matchedMatch, source }) {
  const rows = [];
  const goalScorerMarkets = findGoalScorerMarkets(event);
  const appearances = matchedMatch?.source_match_id
    ? appearancesByMatch.get(matchedMatch.source_match_id) ?? []
    : [];

  for (const market of goalScorerMarkets) {
    ...
    for (const entrant of market.entrants?.nodes ?? []) {
      ...
      const sourceSelectionKey = [
        ...
        sourceMarketId ?? "anytime-goal-scorer",
        entrantId,
      ].join(":");
      ...
      rows.push({
        ...
        matched_laliga_match_id: matchedMatch?.id ?? null,
...
/**
 * Minimal Supabase REST client for LaLiga goal-scorer snapshot reads and writes.
 */
...
/**
 * Fetches current open LaLiga events from the market source.
 */
async function fetchSourceEvents(options) {
  const response = await graphql(TAB_SOURCE, "LaligaGoalScorerMarketSnapshot", LALIGA_GOAL_SCORER_SNAPSHOT_QUERY, {
    category: LALIGA_CATEGORY,
...
/**
 * Fetches official LaLiga fixture rows in the current market event window.
 */
async function fetchOfficialMatchesForEvents(supabase, events) {
  ...
  return await supabase.selectAll("laliga_matches", {
...
async function fetchAppearancesForMatches(supabase, matches) {
  ...
  return await supabase.selectAll("laliga_player_match_appearances", {
...
/**
 * Builds current goal-scorer snapshot rows and matching diagnostics.
 */
function buildSnapshotRows(events, officialMatches, appearances) {
  ...
  for (const event of events) {
    ...
    const matchedMatch = matchExistingLaligaMatch(eventContext, officialMatches);
    ...
    eventSummaries.push({
      eventName: event?.name ?? null,
      matchedLaligaMatchId: matchedMatch?.id ?? null,
...
/**
 * Writes current LaLiga goal-scorer price snapshots to Supabase.
 */
async function writeRows(supabase, rows) {
  await supabase.request("laliga_goal_scorer_market_snapshots", {
    expectJson: false,
    method: "DELETE",
  ...
  await supabase.upsert("laliga_goal_scorer_market_snapshots", rows, "source_selection_key");

  return {
    laligaGoalScorerMarketSnapshots: rows.length,
    ok: true,
    skipped: false,
  };
}
...
/**
 * Summarizes current LaLiga goal-scorer snapshot matching and price coverage.
 */
function summarize({ eventCount, officialMatches, result }) {
  return {
    generatedAt: result.generatedAt,
    matchedEvents: result.eventSummaries.filter((event) => event.matchedLaligaMatchId).length,
    matchedPlayerSelections: result.rows.filter((row) => row.player_source_id).length,
    laligaGoalScorerMarketSnapshots: result.rows.length,
    ...
    unmatchedEvents: result.eventSummaries.filter((event) =>
      !event.reason && !event.matchedLaligaMatchId).length,
...
/**
 * Runs the local LaLiga player goal-scorer market snapshot workflow.
 */
async function main() {
  ...
  if (options.dryRun) {
    console.log(JSON.stringify({
      ...
      sample: result.rows.slice(0, 8).map((row) => ({
        ...
        matchedLaligaMatchId: row.matched_laliga_match_id,
