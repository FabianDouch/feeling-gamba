...
function buildFixedWinRecords(results, matchesById) {
  ...
  for (const row of results) {
    const match = row.matched_mls_match_id ? matchesById.get(row.matched_mls_match_id) : null;
    const base = {
...
function buildFixedWinAggregates(results, matchesById) {
  ...
  for (const record of favouriteRecords) {
    addToBucket(buckets, {
      ...
      scopeKey: "mls:fixed_win_single:overall:favourite",
      scopeType: "overall",
      selectionType: "favourite",
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "fixed_win_single",
        scopeKey: `mls:fixed_win_single:season:${record.season}:favourite`,
        scopeType: "season",
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        ...
        scopeKey: `mls:fixed_win_single:season_round:${record.season}:${record.roundNumber}:favourite`,
  ...
  for (const record of allRecords) {
    addToBucket(buckets, {
      insightType: "fixed_win_single",
      scopeKey: `mls:fixed_win_single:selection_type:${record.selectionType}`,
  ...
  for (const record of favouriteRecords) {
    ...
    addToBucket(buckets, {
      insightType: "fixed_win_single",
      scopeKey: `mls:fixed_win_single:favourite_venue:${record.favouriteVenueSelectionType}`,
  ...
  for (const record of allRecords) {
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      addToBucket(buckets, {
        ...
        scopeKey: `mls:fixed_win_single:price_bucket:${priceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${priceBucket.start.toFixed(2)}`,
    ...
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      for (const priceBucket of getPriceBucketPlusBuckets(record.price, bucketSize)) {
        addToBucket(buckets, {
          ...
          priceBucketLabel: priceBucket.label,
          priceBucketStart: priceBucket.start,
          scopeKey: `mls:fixed_win_single:price_bucket_plus:${priceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${priceBucket.start.toFixed(2)}`,
    ...
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      if (otherPriceBucket) {
        addToBucket(buckets, {
          ...
          scopeKey: `mls:fixed_win_single:other_team_price_bucket:${otherPriceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${otherPriceBucket.start.toFixed(2)}`,
      ...
      for (const otherPricePlusBucket of getPriceBucketPlusBuckets(record.otherPrice, bucketSize)) {
        addToBucket(buckets, {
          ...
          scopeKey: `mls:fixed_win_single:other_team_price_bucket_plus:${otherPricePlusBucket.bucketSize.toFixed(2)}:${record.selectionType}:${otherPricePlusBucket.start.toFixed(2)}`,
      ...
      if (differenceBucket) {
        addToBucket(buckets, {
          ...
          scopeKey: `mls:fixed_win_single:price_difference_bucket:${differenceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${differenceBucket.start.toFixed(2)}`,
      ...
      for (const differencePlusBucket of getPriceDifferencePlusBuckets(record.priceDifference, bucketSize)) {
        addToBucket(buckets, {
          ...
          scopeKey: `mls:fixed_win_single:price_difference_bucket_plus:${differencePlusBucket.bucketSize.toFixed(2)}:${record.selectionType}:${differencePlusBucket.start.toFixed(2)}`,
...
function buildFixedDrawAggregates(results, matchesById) {
  ...
  for (const record of records) {
    addToBucket(buckets, {
      ...
      scopeKey: "mls:fixed_draw_single:overall:draw",
      scopeType: "overall",
    }, record, addFixedWinSelection);
    ...
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      if (priceBucket) {
        addToBucket(buckets, {
          ...
          priceBucketLabel: priceBucket.label,
          priceBucketStart: priceBucket.start,
          scopeKey: `mls:fixed_draw_single:price_bucket:${priceBucket.bucketSize.toFixed(2)}:${priceBucket.start.toFixed(2)}`,
          scopeType: "price_bucket",
        }, record, addFixedWinSelection);
      }
      ...
      for (const priceBucketPlus of getPriceBucketPlusBuckets(record.price, bucketSize)) {
        addToBucket(buckets, {
          ...
          scopeKey: `mls:fixed_draw_single:price_bucket_plus:${priceBucketPlus.bucketSize.toFixed(2)}:${priceBucketPlus.start.toFixed(2)}`,
          scopeType: "price_bucket_plus",
        }, record, addFixedWinSelection);
      }
    }
...
function buildHalfTimeFullTimeAggregates(results, matchesById) {
  ...
  for (const record of favouriteRecords) {
    addToBucket(buckets, {
      ...
      scopeKey: "mls:half_time_full_time_double:overall:favourite",
      scopeType: "overall",
      selectionType: "favourite",
    }, record, addFixedWinSelection);
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "half_time_full_time_double",
        scopeKey: `mls:half_time_full_time_double:season:${record.season}:favourite`,
        scopeType: "season",
        season: record.season,
        selectionType: "favourite",
      }, record, addFixedWinSelection);
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        insightType: "half_time_full_time_double",
        roundNumber: record.roundNumber,
        scopeKey: `mls:half_time_full_time_double:season_round:${record.season}:${record.roundNumber}:favourite`,
        scopeType: "season_round",
        season: record.season,
        selectionType: "favourite",
      }, record, addFixedWinSelection);
  ...
  for (const record of allRecords) {
    addToBucket(buckets, {
      insightType: "half_time_full_time_double",
      scopeKey: `mls:half_time_full_time_double:selection_type:${record.selectionType}`,
      scopeType: "selection_type",
      selectionType: record.selectionType,
    }, record, addFixedWinSelection);
  }
  ...
  for (const record of favouriteRecords) {
    ...
    addToBucket(buckets, {
      insightType: "half_time_full_time_double",
      scopeKey: `mls:half_time_full_time_double:favourite_venue:${record.favouriteVenueSelectionType}`,
      scopeType: "favourite_venue",
      selectionType: record.favouriteVenueSelectionType,
    }, record, addFixedWinSelection);
  }
...
function buildGoalScorerAggregates(appearances, goalScorers, matchesById, matchesBySourceKey, goalScorerPrices) {
  ...
  for (const record of records) {
    addToBucket(buckets, {
      ...
      scopeKey: "mls:goal_scorer_percentage:overall",
      scopeType: "overall",
    ...
    if (record.teamName) {
      addToBucket(buckets, {
        insightType: "goal_scorer_percentage",
        scopeKey: `mls:goal_scorer_percentage:team:${record.teamSourceId ?? record.teamName}`,
        scopeType: "team",
        source: record.source,
    ...
    if (record.playerName) {
      addToBucket(buckets, {
        ...
        scopeKey: `mls:goal_scorer_percentage:player:${record.playerSourceId}`,
        scopeType: "player",
        source: record.source,
      }, record, addGoalScorerAppearance);
    }
    ...
    if (record.playerName && record.teamName) {
      addToBucket(buckets, {
        ...
        playerName: record.playerName,
        playerSourceId: record.playerSourceId,
        scopeKey: `mls:goal_scorer_percentage:player_team:${record.teamSourceId}:${record.playerSourceId}`,
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "goal_scorer_percentage",
        scopeKey: `mls:goal_scorer_percentage:season:${record.season}`,
        scopeType: "season",
        season: record.season,
        source: record.source,
      }, record, addGoalScorerAppearance);
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        insightType: "goal_scorer_percentage",
        roundNumber: record.roundNumber,
        scopeKey: `mls:goal_scorer_percentage:season_round:${record.season}:${record.roundNumber}`,
        scopeType: "season_round",
        season: record.season,
        source: record.source,
      }, record, addGoalScorerAppearance);
  ...
  for (const record of marketRecords) {
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      addToBucket(buckets, {
        ...
        priceBucketLabel: priceBucket.label,
        priceBucketStart: priceBucket.start,
        scopeKey: `mls:goal_scorer_percentage:price_bucket:${priceBucket.bucketSize.toFixed(2)}:${priceBucket.start.toFixed(2)}`,
        scopeType: "price_bucket",
...
function buildSameGameMultiAggregates(results) {
  ...
  for (const row of results) {
    ...
    addToBucket(buckets, {
      ...
      scopeKey: "mls:same_game_multi_percentage:overall:favourite_top2_goal_scorers",
      scopeType: "overall",
      selectionType: "favourite",
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "same_game_multi_percentage",
        scopeKey: `mls:same_game_multi_percentage:season:${record.season}:favourite_top2_goal_scorers`,
        scopeType: "season",
        season: record.season,
        selectionType: "favourite",
      }, record, addSameGameMultiSelection);
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        insightType: "same_game_multi_percentage",
        roundNumber: record.roundNumber,
        scopeKey: `mls:same_game_multi_percentage:season_round:${record.season}:${record.roundNumber}:favourite_top2_goal_scorers`,
        scopeType: "season_round",
        season: record.season,
        selectionType: "favourite",
      }, record, addSameGameMultiSelection);
...
async function readSourceRows(supabase) {
  const [fixedWinResults, matches, appearances, goalScorers, goalScorerPrices, sameGameMultiResults] = await Promise.all([
    supabase.selectAll("mls_fixed_win_snapshot_results", {
    ...
    supabase.selectAll("mls_matches", {
    ...
    supabase.selectAll("mls_player_match_appearances", {
    ...
    supabase.selectAll("mls_goal_scorers", {
    ...
    supabase.selectAll("mls_goal_scorer_market_snapshots", {
    ...
    supabase.selectAll("mls_same_game_multi_results", {
...
/**
 * Removes previous MLS insight rows before replacing the rebuild output.
 */
async function clearExistingAggregates(supabase) {
  await supabase.request("mls_insight_aggregates", {
    expectJson: false,
    method: "DELETE",
...
async function writeAggregates(supabase, rows) {
  ...
  await supabase.upsert("mls_insight_aggregates", rows, "scope_key");

  return {
    mlsInsightAggregates: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(sourceRows, fixedWinRows, fixedDrawRows, goalScorerRows, sameGameMultiRows) {
  return {
    ...
    mlsAppearances: sourceRows.appearances.length,
    mlsMatches: sourceRows.matches.length,
    mlsSameGameMultiResults: sourceRows.sameGameMultiResults.length,
    mlsGoalScorerMarketSnapshots: sourceRows.goalScorerPrices.length,
    mlsGoalScorers: sourceRows.goalScorers.length,
