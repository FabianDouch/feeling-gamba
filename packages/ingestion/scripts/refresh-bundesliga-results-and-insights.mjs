...
/**
 * Maps a calendar date to the Bundesliga season start year.
 */
function getDefaultBundesligaSeasonYear(date) {
...
/**
 * Parses the scheduled LaLiga post-match settlement options.
 */
function parseArgs(argv) {
  ...
  const options = {
    ...
    season: getDefaultBundesligaSeasonYear(now),
    skipInsights: false,
  ...
  for (const arg of argv) {
    ...
    } else if (arg === "--allow-unpriced-backfill") {
...
/**
 * Builds the shared Supabase/write flags used by the child LaLiga workers.
 */
...
/**
 * Builds child refresh commands for official LaLiga settlement and read models.
 */
function buildRefreshCommands(options) {
  ...
  if (options.pricedOnly) {
    resultFlags.push("--priced-only");
  } else {
    resultFlags.push("--allow-unpriced-backfill");
  }

  commands.push(buildCommand("refresh_official_bundesliga_results", "refresh-football-league-results-from-json.mjs", resultFlags));
...
/**
 * Runs official LaLiga settlement and app-facing derived read-model rebuilds.
 */
async function main() {
