...
import { getNationsleagueSeasonYear } from "./lib/nationsleague.mjs";
...
/**
 * Maps a date to the biennial UEFA Nations League ending year.
 */
function getDefaultNationsleagueSeasonYear(date) {
  return getNationsleagueSeasonYear(date);
}

/**
 * Parses the scheduled football cup post-match settlement options.
 */
function parseArgs(argv) {
  ...
  const options = {
    ...
    season: getDefaultNationsleagueSeasonYear(now),
    skipInsights: false,
...
/**
 * Builds the shared Supabase/write flags used by the child football cup workers.
 */
...
/**
 * Builds child refresh commands for official football cup settlement and read models.
 */
function buildRefreshCommands(options) {
  ...
  commands.push(buildCommand("refresh_official_uefa_results", "refresh-nationsleague-results-from-official.mjs", resultFlags));
  ...
  if (!options.skipInsights) {
    commands.push(buildCommand("rebuild_nationsleague_insights", "rebuild-nationsleague-insight-aggregates.mjs", writeFlags));
  ...
  if (!options.skipPredictions) {
    commands.push(buildCommand("generate_nationsleague_single_predictions", "generate-nationsleague-single-predictions.mjs", writeFlags));
  }
...
/**
 * Runs official football cup settlement and app-facing derived read-model rebuilds.
 */
async function main() {
