...
/**
 * Parses the NPC fixed-win snapshot options for the current market source.
 */
...
/**
 * Builds browser-like headers for the NPC market source request.
 */
...
/**
 * Fetches current NPC fixed-win snapshots from the selected public market source.
 */
...
/**
 * Minimal Supabase REST client for NPC market snapshot reads and writes.
 */
...
async function fetchOfficialMatchesForWindow(config, rows, batchSize) {
  ...
  try {
    ...
    if (isMissingNpcSchemaError(error)) {
      return [];
    }
...
function isMissingNpcSchemaError(error) {
  return error instanceof Error
    && error.message.includes("PGRST205")
    && error.message.includes("npc_matches");
}
...
async function rematchRows(config, rows, batchSize) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_npc_match_id: matchExistingNpcMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("npc_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_npc_match_id).length,
    npcMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_npc_match_id).length,
    npcMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
