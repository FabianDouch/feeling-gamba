...
const MODEL_KEY = "laliga_favourite_top2_goal_scorers_same_game_percentage_v1";
...
/**
 * Parses LaLiga same-game multi rebuild options.
 */
...
/**
 * Reads Supabase service-role config for local LaLiga result rebuilds.
 */
...
/**
 * Groups official goal-scorer rows into per-player try counts.
 */
function buildTryCounts(goalScorers) {
...
/**
 * Groups goal-scorer market selections by source event and matched match.
 */
function buildTryPriceBuckets(rows) {
  const buckets = new Map();

  for (const row of rows) {
    const key = `${row.source}:${row.matched_laliga_match_id ?? ""}:${row.source_event_id}`;
...
function selectTopTwoGoalScorers({ fixedWinRow, favouriteTeamSourceId, tryPriceBuckets }) {
  const key = `${fixedWinRow.source}:${fixedWinRow.matched_laliga_match_id ?? ""}:${fixedWinRow.source_event_id}`;
...
function getFixedWinModelKey(row) {
  ...
  if (row.matched_laliga_match_id) {
    return `${row.source}:match:${row.matched_laliga_match_id}`;
  }
...
function compareFixedWinRowsForModel(left, right) {
  const matchedDiff = Number(Boolean(right.matched_laliga_match_id)) - Number(Boolean(left.matched_laliga_match_id));
...
function buildSameGameMultiRows({ fixedWinResults, matchesById, tryPriceBuckets, tryCounts }) {
  return selectFixedWinRowsForModel(fixedWinResults).map((fixedWinRow) => {
    const match = fixedWinRow.matched_laliga_match_id
      ? matchesById.get(fixedWinRow.matched_laliga_match_id)
      : null;
    ...
    const first = selectedGoalScorers[0] ?? null;
    const second = selectedGoalScorers[1] ?? null;
    ...
    return {
      ...
      match_label: fixedWinRow.home_team_name && fixedWinRow.away_team_name
        ...
        : null,
      matched_laliga_match_id: fixedWinRow.matched_laliga_match_id,
      model_key: MODEL_KEY,
      ...
      raw: {
        ...
        returnBasis: "estimated_product_of_fixed_win_and_two_goal_scorer_prices",
      },
      ...
      goal_scorer_1_name: first?.player_name ?? null,
      goal_scorer_1_player_source_id: first?.player_source_id ?? null,
      goal_scorer_1_price: firstPrice,
      goal_scorer_1_scored: firstTryCount === null ? null : firstTryCount > 0,
      goal_scorer_1_snapshot_id: first?.id ?? null,
      goal_scorer_1_goal_count: firstTryCount,
      goal_scorer_2_name: second?.player_name ?? null,
      goal_scorer_2_player_source_id: second?.player_source_id ?? null,
      goal_scorer_2_price: secondPrice,
      goal_scorer_2_scored: secondTryCount === null ? null : secondTryCount > 0,
      goal_scorer_2_snapshot_id: second?.id ?? null,
      goal_scorer_2_goal_count: secondTryCount,
    };
  });
}

/**
 * Loads fixed-win, goal-scorer price, match, and result rows for the rebuild.
 */
...
/**
 * Rlaligaaces the stored same-game multi rows for the current model.
 */
async function writeRows(supabase, rows) {
  await supabase.request("laliga_same_game_multi_results", {
    expectJson: false,
    method: "DELETE",
  ...
  await supabase.upsert("laliga_same_game_multi_results", rows, "source_result_key");

  return {
    laligaSameGameMultiResults: rows.length,
    ok: true,
    skipped: false,
  };
}
...
/**
 * Runs the local LaLiga same-game multi result rebuild workflow.
 */
async function main() {
  ...
  const rows = buildSameGameMultiRows({
    ...
    tryCounts: buildTryCounts(sourceRows.goalScorers),
    tryPriceBuckets: buildTryPriceBuckets(sourceRows.goalScorerPrices),
  });
  const summary = {
    ...
    laligaFixedWinSnapshotResults: sourceRows.fixedWinResults.length,
    laligaSameGameMultiResults: rows.length,
