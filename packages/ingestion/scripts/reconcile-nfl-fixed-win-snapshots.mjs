...
/**
 * Minimal Supabase REST client for NFL fixed-win reconciliation.
 */
...
/**
 * Confirms an official NFL match has final scores usable for settlement.
 */
...
function mapOutcome(snapshot, match) {
  if (!snapshot.matched_nfl_match_id) {
    return {
...
function buildResultRow(snapshot, match, winner, outcome) {
  return {
    ...
    matched_nfl_match_id: snapshot.matched_nfl_match_id,
...
async function readSnapshots(supabase, options) {
  ...
  const rows = await supabase.request("nfl_market_snapshots", {
    search,
  });
...
/**
 * Loads official NFL matches referenced by the selected snapshots.
 */
...
/**
 * Builds fixed-win outcome rows from source snapshots and official NFL results.
 */
...
async function writeRows(supabase, rows) {
  await supabase.upsert(
    "nfl_fixed_win_snapshot_results",
    rows,
    "source_snapshot_key",
  );

  return {
    nflFixedWinSnapshotResults: rows.length,
    ok: true,
    skipped: false,
  };
}
