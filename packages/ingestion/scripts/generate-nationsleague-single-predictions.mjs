...
const FIXED_WIN_MODEL = "nationsleague_fixed_win_percentage_single_v1";
const GOAL_SCORER_MODEL = "nationsleague_goal_scorer_percentage_single_v1";
const GOAL_SCORER_CANDIDATES_PER_TEAM = 3;
const TEAM_NAME_ALIASES = NATIONSLEAGUE_TEAM_ALIASES;
const NATIONSLEAGUE_SINGLE_PREDICTION_COLUMNS = [
  "advertised_start_at",
  "away_team_name",
  "bucket_sample_size",
  "home_team_name",
...
/**
 * Applies explicit TAB-vs-UEFA Nations League club aliases after basic name normalization.
 */
...
/**
 * Builds team records from stored official settled Nations League matches.
 */
function buildTeamRecords(matches) {
  ...
  for (const match of matches) {
    ...
    addTeam(match.away_team_source_id, match.away_team_name, awayWon);
  }
...
function createTeamSignal(record) {
  ...
  return {
    detail: `${wins} wins from ${selections} stored settled matches`,
    score,
    tone,
  };
}
...
function buildFixedWinPredictions({ matchesById, predictedAt, snapshots, sourceDate, teamRecords }) {
  ...
  for (const snapshot of getLatestSnapshots(snapshots)) {
    if (!snapshot.favourite_team_name || !snapshot.matched_nationsleague_match_id) {
      continue;
    }

    const match = matchesById.get(snapshot.matched_nationsleague_match_id);
    ...
    rows.push({
      ...
      matched_nationsleague_match_id: snapshot.matched_nationsleague_match_id,
      ...
      raw: {
        scoreSource: "official_uefa_team_win_percentage",
...
/**
 * Selects the strongest historical goal-scorer candidates for each upcoming team.
 */
function getTopGoalScorerRowsByTeam(aggregates) {
  const byTeam = new Map();

  for (const row of aggregates) {
    if (
      row.insight_type !== "goal_scorer_percentage"
  ...
  for (const [teamSourceId, teamRows] of byTeam.entries()) {
    ...
    byTeam.set(teamSourceId, teamRows.slice(0, GOAL_SCORER_CANDIDATES_PER_TEAM));
  }
...
/**
 * Builds current goal-scorer percentage singles from upcoming teams and historical player/team rates.
 */
function buildGoalScorerPredictions({ aggregates, matches, predictedAt, sourceDate }) {
  const rows = [];
  const goalScorersByTeam = getTopGoalScorerRowsByTeam(aggregates);
  ...
  for (const match of matches) {
    ...
    for (const team of teams) {
      const candidates = team.teamSourceId
        ? goalScorersByTeam.get(team.teamSourceId) ?? []
        : [];
      ...
      for (const candidate of candidates) {
        ...
        rows.push({
          ...
          matched_nationsleague_match_id: match.id,
          ...
          predicted_team_name: team.teamName,
          predicted_team_source_id: team.teamSourceId,
          prediction_model: GOAL_SCORER_MODEL,
          prediction_rank: null,
          prediction_signature: [
          ...
          raw: {
            scoreSource: "official_uefa_player_team_goal_scorer_percentage",
            totalGoals: candidate.total_goals,
          },
...
function normalizeRowForWrite(row) {
  return Object.fromEntries(NATIONSLEAGUE_SINGLE_PREDICTION_COLUMNS.map((column) => [
    column,
    row[column] ?? null,
  ]));
}
...
async function readSourceRows(supabase) {
  const [snapshots, matches, aggregates] = await Promise.all([
    supabase.selectAll("nationsleague_market_snapshots", {
    ...
    supabase.selectAll("nationsleague_insight_aggregates", {
...
async function writeRows(supabase, rows, sourceDate) {
  await supabase.request("nationsleague_single_predictions", {
    expectJson: false,
    method: "DELETE",
    prefer: "return=minimal",
    search: {
      prediction_model: `in.(${FIXED_WIN_MODEL},${GOAL_SCORER_MODEL})`,
      source_date: `eq.${sourceDate}`,
    },
  });

  await supabase.upsert(
    "nationsleague_single_predictions",
    rows.map(normalizeRowForWrite),
    "source_prediction_key",
  );

  return {
    nationsleagueSinglePredictions: rows.length,
    ok: true,
    skipped: false,
  };
}
 ...
 */
function summarize(sourceRows, fixedWinRows, goalScorerRows) {
  return {
    fixedWinPredictions: fixedWinRows.length,
    sourceMarketSnapshots: sourceRows.snapshots.length,
    sourceNationsleagueInsightAggregates: sourceRows.aggregates.length,
    sourceNationsleagueMatches: sourceRows.matches.length,
...
async function main() {
  ...
  const goalScorerRows = buildGoalScorerPredictions({
    aggregates: sourceRows.aggregates,
    matches: sourceRows.matches,
    predictedAt,
    sourceDate: options.sourceDate,
  });
  const rows = [...fixedWinRows, ...goalScorerRows];
  const summary = summarize(sourceRows, fixedWinRows, goalScorerRows);
