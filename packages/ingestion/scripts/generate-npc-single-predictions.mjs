...
const FIXED_WIN_MODEL = "npc_fixed_win_percentage_single_v1";
const TRY_SCORER_MODEL = "npc_try_scorer_percentage_single_v1";
const TRY_SCORER_CANDIDATES_PER_TEAM = 3;
const NPC_SINGLE_PREDICTION_COLUMNS = [
  "advertised_start_at",
  "away_team_name",
  "bucket_sample_size",
  "home_team_name",
...
/**
 * Parses NPC single prediction generation options.
 */
...
/**
 * Minimal Supabase REST client for NPC prediction generation.
 */
...
/**
 * Builds season-to-date team records from official settled NPC matches.
 */
...
function buildFixedWinPredictions({ matchesById, predictedAt, snapshots, sourceDate, teamRecords }) {
  ...
  for (const snapshot of getLatestSnapshots(snapshots)) {
    if (!snapshot.favourite_team_name || !snapshot.matched_npc_match_id) {
      continue;
    }

    const match = matchesById.get(snapshot.matched_npc_match_id);
    ...
    for (const team of teams) {
      ...
      for (const candidate of candidates) {
        ...
        rows.push({
          ...
          source: "official_provincial_rugby",
          source_date: sourceDate,
          source_event_id: match.source_match_id,
          source_match_id: match.source_match_id,
          source_prediction_key: [
            TRY_SCORER_MODEL,
            "official_provincial_rugby",
            sourceDate,
            match.source_match_id,
            candidate.player_source_id,
          ].join(":"),
...
function normalizeRowForWrite(row) {
  return Object.fromEntries(NPC_SINGLE_PREDICTION_COLUMNS.map((column) => [
    column,
    row[column] ?? null,
  ]));
}
...
/**
 * Loads source rows needed by both NPC single prediction models.
 */
async function readSourceRows(supabase) {
  const [snapshots, matches, aggregates] = await Promise.all([
    supabase.selectAll("npc_market_snapshots", {
    ...
    supabase.selectAll("npc_insight_aggregates", {
...
/**
 * Writes generated NPC single predictions to Supabase.
 */
async function writeRows(supabase, rows, sourceDate) {
  await supabase.request("npc_single_predictions", {
    expectJson: false,
    method: "DELETE",
  ...
  return {
    npcSinglePredictions: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(sourceRows, fixedWinRows, tryScorerRows) {
  return {
    ...
    sourceNpcInsightAggregates: sourceRows.aggregates.length,
    sourceNpcMatches: sourceRows.matches.length,
    tryScorerPredictions: tryScorerRows.length,
  };
}

/**
 * Runs the local NPC single prediction generation workflow.
 */
async function main() {
