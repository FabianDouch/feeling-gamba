...
const FIXED_WIN_MODEL = "seriea_fixed_win_percentage_single_v1";
const GOAL_SCORER_MODEL = "seriea_goal_scorer_percentage_single_v1";
...
const SERIEA_SINGLE_PREDICTION_COLUMNS = [
  "advertised_start_at",
  "away_team_name",
  "bucket_sample_size",
  "home_team_name",
...
/**
 * Applies explicit TAB-vs-Serie A club aliases after basic name normalization.
 */
...
function buildFixedWinPredictions({ matchesById, predictedAt, snapshots, sourceDate, teamRecords }) {
  ...
  for (const snapshot of getLatestSnapshots(snapshots)) {
    if (!snapshot.favourite_team_name || !snapshot.matched_seriea_match_id) {
      continue;
    }

    const match = matchesById.get(snapshot.matched_seriea_match_id);
    ...
    rows.push({
      ...
      matched_seriea_match_id: snapshot.matched_seriea_match_id,
    ...
    for (const team of teams) {
      ...
      for (const candidate of candidates) {
        ...
        rows.push({
          ...
          matched_seriea_match_id: match.id,
          ...
          raw: {
            scoreSource: "official_2026_player_team_goal_scorer_percentage",
...
function normalizeRowForWrite(row) {
  return Object.fromEntries(SERIEA_SINGLE_PREDICTION_COLUMNS.map((column) => [
    column,
    row[column] ?? null,
  ]));
}
...
async function readSourceRows(supabase) {
  const [snapshots, matches, aggregates] = await Promise.all([
    supabase.selectAll("seriea_market_snapshots", {
    ...
    supabase.selectAll("seriea_insight_aggregates", {
...
async function writeRows(supabase, rows, sourceDate) {
  await supabase.request("seriea_single_predictions", {
    expectJson: false,
    method: "DELETE",
  ...
  await supabase.upsert(
    "seriea_single_predictions",
    rows.map(normalizeRowForWrite),
    "source_prediction_key",
  );

  return {
    serieaSinglePredictions: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(sourceRows, fixedWinRows, goalScorerRows) {
  return {
    ...
    sourceSerieaInsightAggregates: sourceRows.aggregates.length,
    sourceSerieaMatches: sourceRows.matches.length,
