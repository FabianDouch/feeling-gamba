...
const MLS_CATEGORY = "SOCCER";
const MLS_COMPETITION_SLUG = "major-league-soccer";
const MLS_FIXED_WIN_MARKET_NAME = "match result";
const TAB_SOURCE = {
  endpoint: "https://api.tab.co.nz/graphql",
  label: "MLS market source",
  source: "tab",
};
const TEAM_NAME_ALIASES = new Map();

const MLS_COMPETITION_QUERY = `
...
/**
 * Parses the MLS fixed-win snapshot options for the current market source.
 */
function parseArgs(argv) {
  const options = {
    batchSize: DEFAULT_BATCH_SIZE,
    competitionSlug: MLS_COMPETITION_SLUG,
    dryRun: false,
...
/**
 * Builds browser-like headers for the MLS market source request.
 */
...
function findFixedWinMarket(event) {
  return (event.markets?.nodes ?? []).find((market) =>
    normalizeName(market?.name) === MLS_FIXED_WIN_MARKET_NAME);
}
...
function matchExistingMlsMatch(snapshot, matches) {
...
function mapSnapshot(source, event, generatedAt, officialMatches) {
  ...
  const matchedMatch = matchExistingMlsMatch(snapshot, officialMatches);

  return {
    ...snapshot,
    matched_mls_match_id: matchedMatch?.id ?? null,
  };
}

/**
 * Fetches current MLS fixed-win snapshots from the selected public market source.
 */
async function fetchSnapshots(options, officialMatches) {
  ...
  for (const source of [TAB_SOURCE]) {
    const response = await graphql(source, "SportingCompetitionScreen", MLS_COMPETITION_QUERY, {
      category: MLS_CATEGORY,
...
/**
 * Minimal Supabase REST client for MLS market snapshot reads and writes.
 */
...
function isMissingMlsSchemaError(error) {
  return error instanceof Error
    && error.message.includes("PGRST205")
    && error.message.includes("mls_matches");
}
...
async function rematchRows(config, rows, batchSize) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_mls_match_id: matchExistingMlsMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("mls_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_mls_match_id).length,
    mlsMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_mls_match_id).length,
    mlsMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
  ...
  if (options.dryRun) {
    console.log(JSON.stringify({
      ...
      sample: matchedRows.slice(0, 5).map((row) => ({
        ...
        matchedMlsMatchId: row.matched_mls_match_id,
        source: row.source,
