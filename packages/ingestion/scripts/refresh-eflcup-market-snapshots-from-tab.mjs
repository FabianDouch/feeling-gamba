...
const EFLCUP_CATEGORY = "SOCCER";
const EFLCUP_COMPETITION_SLUG = "english-league-cup";
const EFLCUP_FIXED_WIN_MARKET_NAME = "match result";
const TAB_SOURCE = {
  endpoint: "https://api.tab.co.nz/graphql",
  label: "EFL Cup market source",
  source: "tab",
};
const TEAM_NAME_ALIASES = new Map();

const EFLCUP_COMPETITION_QUERY = `
...
/**
 * Parses the EFL Cup fixed-win snapshot options for the current market source.
 */
function parseArgs(argv) {
  const options = {
    batchSize: DEFAULT_BATCH_SIZE,
    competitionSlug: EFLCUP_COMPETITION_SLUG,
    dryRun: false,
...
/**
 * Builds browser-like headers for the EFL Cup market source request.
 */
...
function findFixedWinMarket(event) {
  return (event.markets?.nodes ?? []).find((market) =>
    normalizeName(market?.name) === EFLCUP_FIXED_WIN_MARKET_NAME);
}
...
function matchExistingEflcupMatch(snapshot, matches) {
...
function mapSnapshot(source, event, generatedAt, officialMatches) {
  ...
  const matchedMatch = matchExistingEflcupMatch(snapshot, officialMatches);

  return {
    ...snapshot,
    matched_eflcup_match_id: matchedMatch?.id ?? null,
  };
}

/**
 * Fetches current EFL Cup fixed-win snapshots from the selected public market source.
 */
async function fetchSnapshots(options, officialMatches) {
  ...
  for (const source of [TAB_SOURCE]) {
    const response = await graphql(source, "SportingCompetitionScreen", EFLCUP_COMPETITION_QUERY, {
      category: EFLCUP_CATEGORY,
...
/**
 * Minimal Supabase REST client for EFL Cup market snapshot reads and writes.
 */
...
function isMissingEflcupSchemaError(error) {
  return error instanceof Error
    && error.message.includes("PGRST205")
    && error.message.includes("eflcup_matches");
}
...
async function rematchRows(config, rows, batchSize) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_eflcup_match_id: matchExistingEflcupMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("eflcup_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_eflcup_match_id).length,
    eflcupMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_eflcup_match_id).length,
    eflcupMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
  ...
  if (options.dryRun) {
    console.log(JSON.stringify({
      ...
      sample: matchedRows.slice(0, 5).map((row) => ({
        ...
        matchedEflcupMatchId: row.matched_eflcup_match_id,
        source: row.source,
