...
/**
 * Parses football cup aggregate rebuild options.
 */
...
/**
 * Minimal Supabase REST client for football cup aggregate rebuilds.
 */
...
/**
 * Gets the official match date used for football cup aggregate date ranges.
 */
...
/**
 * Adds one player appearance to a goal-scorer percentage aggregate bucket.
 */
function addGoalScorerAppearance(bucket, record) {
  addEvent(bucket, record);
  ...
  bucket.selection_count += 1;
  bucket.total_goals += record.tryCount;
...
/**
 * Adds one priced player goal-scorer market selection to an aggregate bucket.
 */
function addGoalScorerMarketSelection(bucket, record) {
  addEvent(bucket, record);
...
function buildFixedWinRecords(results, matchesById) {
  ...
  for (const row of results) {
    const match = row.matched_nationsleague_match_id ? matchesById.get(row.matched_nationsleague_match_id) : null;
    const base = {
...
function buildFixedWinAggregates(results, matchesById) {
  ...
  for (const record of favouriteRecords) {
    addToBucket(buckets, {
      ...
      scopeKey: "nationsleague:fixed_win_single:overall:favourite",
      scopeType: "overall",
      selectionType: "favourite",
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "fixed_win_single",
        scopeKey: `nationsleague:fixed_win_single:season:${record.season}:favourite`,
        scopeType: "season",
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        ...
        scopeKey: `nationsleague:fixed_win_single:season_round:${record.season}:${record.roundNumber}:favourite`,
  ...
  for (const record of allRecords) {
    addToBucket(buckets, {
      insightType: "fixed_win_single",
      scopeKey: `nationsleague:fixed_win_single:selection_type:${record.selectionType}`,
  ...
  for (const record of favouriteRecords) {
    ...
    addToBucket(buckets, {
      insightType: "fixed_win_single",
      scopeKey: `nationsleague:fixed_win_single:favourite_venue:${record.favouriteVenueSelectionType}`,
  ...
  for (const record of allRecords) {
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      addToBucket(buckets, {
        ...
        scopeKey: `nationsleague:fixed_win_single:price_bucket:${priceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${priceBucket.start.toFixed(2)}`,
    ...
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      for (const priceBucket of getPriceBucketPlusBuckets(record.price, bucketSize)) {
        addToBucket(buckets, {
          ...
          priceBucketLabel: priceBucket.label,
          priceBucketStart: priceBucket.start,
          scopeKey: `nationsleague:fixed_win_single:price_bucket_plus:${priceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${priceBucket.start.toFixed(2)}`,
    ...
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      if (otherPriceBucket) {
        addToBucket(buckets, {
          ...
          scopeKey: `nationsleague:fixed_win_single:other_team_price_bucket:${otherPriceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${otherPriceBucket.start.toFixed(2)}`,
      ...
      for (const otherPricePlusBucket of getPriceBucketPlusBuckets(record.otherPrice, bucketSize)) {
        addToBucket(buckets, {
          ...
          scopeKey: `nationsleague:fixed_win_single:other_team_price_bucket_plus:${otherPricePlusBucket.bucketSize.toFixed(2)}:${record.selectionType}:${otherPricePlusBucket.start.toFixed(2)}`,
      ...
      if (differenceBucket) {
        addToBucket(buckets, {
          ...
          scopeKey: `nationsleague:fixed_win_single:price_difference_bucket:${differenceBucket.bucketSize.toFixed(2)}:${record.selectionType}:${differenceBucket.start.toFixed(2)}`,
      ...
      for (const differencePlusBucket of getPriceDifferencePlusBuckets(record.priceDifference, bucketSize)) {
        addToBucket(buckets, {
          ...
          scopeKey: `nationsleague:fixed_win_single:price_difference_bucket_plus:${differencePlusBucket.bucketSize.toFixed(2)}:${record.selectionType}:${differencePlusBucket.start.toFixed(2)}`,
...
/**
 * Builds draw-selection records from settled three-way football cup fixed-win results.
 */
...
function buildFixedDrawAggregates(results, matchesById) {
  ...
  for (const record of records) {
    addToBucket(buckets, {
      ...
      scopeKey: "nationsleague:fixed_draw_single:overall:draw",
      scopeType: "overall",
    }, record, addFixedWinSelection);
    ...
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      if (priceBucket) {
        addToBucket(buckets, {
          ...
          priceBucketLabel: priceBucket.label,
          priceBucketStart: priceBucket.start,
          scopeKey: `nationsleague:fixed_draw_single:price_bucket:${priceBucket.bucketSize.toFixed(2)}:${priceBucket.start.toFixed(2)}`,
          scopeType: "price_bucket",
        }, record, addFixedWinSelection);
      }
      ...
      for (const priceBucketPlus of getPriceBucketPlusBuckets(record.price, bucketSize)) {
        addToBucket(buckets, {
          ...
          scopeKey: `nationsleague:fixed_draw_single:price_bucket_plus:${priceBucketPlus.bucketSize.toFixed(2)}:${priceBucketPlus.start.toFixed(2)}`,
          scopeType: "price_bucket_plus",
        }, record, addFixedWinSelection);
      }
    }
...
function buildHalfTimeFullTimeAggregates(results, matchesById) {
  ...
  for (const record of favouriteRecords) {
    addToBucket(buckets, {
      ...
      scopeKey: "nationsleague:half_time_full_time_double:overall:favourite",
      scopeType: "overall",
      selectionType: "favourite",
    }, record, addFixedWinSelection);
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "half_time_full_time_double",
        scopeKey: `nationsleague:half_time_full_time_double:season:${record.season}:favourite`,
        scopeType: "season",
        season: record.season,
        selectionType: "favourite",
      }, record, addFixedWinSelection);
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        insightType: "half_time_full_time_double",
        roundNumber: record.roundNumber,
        scopeKey: `nationsleague:half_time_full_time_double:season_round:${record.season}:${record.roundNumber}:favourite`,
        scopeType: "season_round",
        season: record.season,
        selectionType: "favourite",
      }, record, addFixedWinSelection);
  ...
  for (const record of allRecords) {
    addToBucket(buckets, {
      insightType: "half_time_full_time_double",
      scopeKey: `nationsleague:half_time_full_time_double:selection_type:${record.selectionType}`,
      scopeType: "selection_type",
      selectionType: record.selectionType,
    }, record, addFixedWinSelection);
  }
  ...
  for (const record of favouriteRecords) {
    ...
    addToBucket(buckets, {
      insightType: "half_time_full_time_double",
      scopeKey: `nationsleague:half_time_full_time_double:favourite_venue:${record.favouriteVenueSelectionType}`,
      scopeType: "favourite_venue",
      selectionType: record.favouriteVenueSelectionType,
    }, record, addFixedWinSelection);
  }
...
/**
 * Builds appearance-level goal-scorer records from official rosters and goal events.
 */
function buildGoalScorerRecords(appearances, goalScorers, matchesBySourceKey) {
...
/**
 * Builds priced goal-scorer market records with official football cup settlement.
 */
function buildGoalScorerMarketRecords(goalScorerPrices, goalScorers, matchesById) {
  const tryCounts = new Map();
  ...
  return goalScorerPrices.map((row) => {
    const match = row.matched_nationsleague_match_id ? matchesById.get(row.matched_nationsleague_match_id) : null;
...
/**
 * Builds goal-scorer aggregate rows from official appearances and captured prices.
 */
function buildGoalScorerAggregates(appearances, goalScorers, matchesById, matchesBySourceKey, goalScorerPrices) {
  const buckets = new Map();
  const records = buildGoalScorerRecords(appearances, goalScorers, matchesBySourceKey);
  const marketRecords = buildGoalScorerMarketRecords(goalScorerPrices, goalScorers, matchesById);

  for (const record of records) {
    addToBucket(buckets, {
      insightType: "goal_scorer_percentage",
      scopeKey: "nationsleague:goal_scorer_percentage:overall",
      scopeType: "overall",
      source: record.source,
    }, record, addGoalScorerAppearance);

    if (record.teamName) {
      addToBucket(buckets, {
        insightType: "goal_scorer_percentage",
        scopeKey: `nationsleague:goal_scorer_percentage:team:${record.teamSourceId ?? record.teamName}`,
        scopeType: "team",
        source: record.source,
    ...
    if (record.playerName) {
      addToBucket(buckets, {
        ...
        scopeKey: `nationsleague:goal_scorer_percentage:player:${record.playerSourceId}`,
        scopeType: "player",
        source: record.source,
      }, record, addGoalScorerAppearance);
    }
    ...
    if (record.playerName && record.teamName) {
      addToBucket(buckets, {
        ...
        playerName: record.playerName,
        playerSourceId: record.playerSourceId,
        scopeKey: `nationsleague:goal_scorer_percentage:player_team:${record.teamSourceId}:${record.playerSourceId}`,
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "goal_scorer_percentage",
        scopeKey: `nationsleague:goal_scorer_percentage:season:${record.season}`,
        scopeType: "season",
        season: record.season,
        source: record.source,
      }, record, addGoalScorerAppearance);
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        insightType: "goal_scorer_percentage",
        roundNumber: record.roundNumber,
        scopeKey: `nationsleague:goal_scorer_percentage:season_round:${record.season}:${record.roundNumber}`,
        scopeType: "season_round",
        season: record.season,
        source: record.source,
      }, record, addGoalScorerAppearance);
  ...
  for (const record of marketRecords) {
    for (const bucketSize of PRICE_BUCKET_SIZES) {
      ...
      addToBucket(buckets, {
        ...
        priceBucketLabel: priceBucket.label,
        priceBucketStart: priceBucket.start,
        scopeKey: `nationsleague:goal_scorer_percentage:price_bucket:${priceBucket.bucketSize.toFixed(2)}:${priceBucket.start.toFixed(2)}`,
        scopeType: "price_bucket",
      }, record, addGoalScorerMarketSelection);
    }
  }
...
function buildSameGameMultiAggregates(results) {
  ...
  for (const row of results) {
    ...
    addToBucket(buckets, {
      ...
      scopeKey: "nationsleague:same_game_multi_percentage:overall:favourite_top2_goal_scorers",
      scopeType: "overall",
      selectionType: "favourite",
    ...
    if (record.season) {
      addToBucket(buckets, {
        insightType: "same_game_multi_percentage",
        scopeKey: `nationsleague:same_game_multi_percentage:season:${record.season}:favourite_top2_goal_scorers`,
        scopeType: "season",
        season: record.season,
        selectionType: "favourite",
      }, record, addSameGameMultiSelection);
    ...
    if (record.season && record.roundNumber) {
      addToBucket(buckets, {
        insightType: "same_game_multi_percentage",
        roundNumber: record.roundNumber,
        scopeKey: `nationsleague:same_game_multi_percentage:season_round:${record.season}:${record.roundNumber}:favourite_top2_goal_scorers`,
        scopeType: "season_round",
        season: record.season,
        selectionType: "favourite",
      }, record, addSameGameMultiSelection);
...
/**
 * Loads all source rows needed for football cup insight rebuilds.
 */
async function readSourceRows(supabase) {
  const [fixedWinResults, matches, appearances, goalScorers, goalScorerPrices, sameGameMultiResults] = await Promise.all([
    supabase.selectAll("nationsleague_fixed_win_snapshot_results", {
    ...
    supabase.selectAll("nationsleague_matches", {
    ...
    supabase.selectAll("nationsleague_player_match_appearances", {
    ...
    supabase.selectAll("nationsleague_goal_scorers", {
    ...
    supabase.selectAll("nationsleague_goal_scorer_market_snapshots", {
    ...
    supabase.selectAll("nationsleague_same_game_multi_results", {
...
/**
 * Removes previous UEFA Nations League insight rows before replacing the rebuild output.
 */
async function clearExistingAggregates(supabase) {
  await supabase.request("nationsleague_insight_aggregates", {
    expectJson: false,
    method: "DELETE",
...
/**
 * Writes football cup insight aggregate rows to Supabase.
 */
async function writeAggregates(supabase, rows) {
  await clearExistingAggregates(supabase);
  await supabase.upsert("nationsleague_insight_aggregates", rows, "scope_key");

  return {
    nationsleagueInsightAggregates: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(sourceRows, fixedWinRows, fixedDrawRows, goalScorerRows, sameGameMultiRows) {
  return {
    ...
    nationsleagueAppearances: sourceRows.appearances.length,
    nationsleagueMatches: sourceRows.matches.length,
    nationsleagueSameGameMultiResults: sourceRows.sameGameMultiResults.length,
    nationsleagueGoalScorerMarketSnapshots: sourceRows.goalScorerPrices.length,
    nationsleagueGoalScorers: sourceRows.goalScorers.length,
    sameGameMultiAggregateRows: sameGameMultiRows.length,
    goalScorerAggregateRows: goalScorerRows.length,
  };
}

/**
 * Runs the local football cup insight aggregate rebuild workflow.
 */
async function main() {
