...
const MODEL_KEY = "nrl_favourite_top2_try_scorers_same_game_percentage_v1";

/**
 * Parses NRL same-game multi rebuild options.
 */
...
/**
 * Reads Supabase service-role config for local NRL result rebuilds.
 */
...
function buildTryPriceBuckets(rows) {
  ...
  for (const row of rows) {
    const key = `${row.source}:${row.matched_nrl_match_id ?? ""}:${row.source_event_id}`;
...
function selectTopTwoTryScorers({ fixedWinRow, favouriteTeamSourceId, tryPriceBuckets }) {
  const key = `${fixedWinRow.source}:${fixedWinRow.matched_nrl_match_id ?? ""}:${fixedWinRow.source_event_id}`;
...
function getFixedWinModelKey(row) {
  ...
  if (row.matched_nrl_match_id) {
    return `${row.source}:match:${row.matched_nrl_match_id}`;
  }
...
function compareFixedWinRowsForModel(left, right) {
  const matchedDiff = Number(Boolean(right.matched_nrl_match_id)) - Number(Boolean(left.matched_nrl_match_id));
...
function buildSameGameMultiRows({ fixedWinResults, matchesById, tryPriceBuckets, tryCounts }) {
  return selectFixedWinRowsForModel(fixedWinResults).map((fixedWinRow) => {
    const match = fixedWinRow.matched_nrl_match_id
      ? matchesById.get(fixedWinRow.matched_nrl_match_id)
      : null;
    ...
    return {
      ...
      match_label: fixedWinRow.home_team_name && fixedWinRow.away_team_name
        ...
        : null,
      matched_nrl_match_id: fixedWinRow.matched_nrl_match_id,
      model_key: MODEL_KEY,
...
async function writeRows(supabase, rows) {
  await supabase.request("nrl_same_game_multi_results", {
    expectJson: false,
    method: "DELETE",
  ...
  await supabase.upsert("nrl_same_game_multi_results", rows, "source_result_key");

  return {
    nrlSameGameMultiResults: rows.length,
    ok: true,
    skipped: false,
  };
}
...
/**
 * Runs the local NRL same-game multi result rebuild workflow.
 */
async function main() {
  ...
  const summary = {
    ...
    nrlFixedWinSnapshotResults: sourceRows.fixedWinResults.length,
    nrlSameGameMultiResults: rows.length,
