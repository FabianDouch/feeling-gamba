...
const DEFAULT_COMPETITION_SLUG = "english-league-cup";

/**
 * Parses the EFL Cup current-market refresh orchestration options.
 */
...
/**
 * Builds the shared Supabase/write flags used by the child EFL Cup workers.
 */
...
/**
 * Builds the ordered EFL Cup current-market refresh command list.
 */
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_eflcup_fixed_win_markets", "refresh-eflcup-market-snapshots-from-tab.mjs", [
  ...
  if (!options.skipGoalScorers || !options.skipReconcile || !options.skipPredictions) {
    commands.push(buildCommand("refresh_priced_eflcup_official_matches", "refresh-football-league-results-from-json.mjs", [
      "--league=eflcup",
      "--include-fixtures",
      "--priced-only",
      ...writeFlags,
  ...
  if (!options.skipGoalScorers) {
    commands.push(buildCommand("capture_eflcup_goal_scorer_markets", "refresh-eflcup-goal-scorer-market-snapshots-from-tab.mjs", [
      `--competition-slug=${options.competitionSlug}`,
  ...
  if (!options.skipReconcile) {
    commands.push(buildCommand("reconcile_eflcup_fixed_win", "reconcile-eflcup-fixed-win-snapshots.mjs", writeFlags));
  }

  if (!options.skipInsights) {
    commands.push(buildCommand("rebuild_eflcup_insights", "rebuild-eflcup-insight-aggregates.mjs", writeFlags));
  }

  if (!options.skipPredictions) {
    commands.push(buildCommand("generate_eflcup_single_predictions", "generate-eflcup-single-predictions.mjs", writeFlags));
  }
...
/**
 * Runs the EFL Cup current-market capture and derived read-model refresh pipeline.
 */
async function main() {
