...
const LALIGA_COMPETITION_QUERY = `
...
/**
 * Parses the LaLiga fixed-win snapshot options for the current market source.
 */
...
/**
 * Builds browser-like headers for the LaLiga market source request.
 */
...
function matchExistingLaligaMatch(snapshot, matches) {
...
function mapSnapshot(source, event, generatedAt, officialMatches) {
  ...
  const matchedMatch = matchExistingLaligaMatch(snapshot, officialMatches);
...
/**
 * Fetches current LaLiga fixed-win snapshots from the selected public market source.
 */
async function fetchSnapshots(options, officialMatches) {
  ...
  for (const source of [TAB_SOURCE]) {
    const response = await graphql(source, "SportingCompetitionScreen", LALIGA_COMPETITION_QUERY, {
...
/**
 * Minimal Supabase REST client for LaLiga market snapshot reads and writes.
 */
...
async function fetchOfficialMatchesForWindow(config, rows, batchSize) {
  ...
  try {
    return await supabase.request("laliga_matches", {
      search: {
...
function isMissingLaligaSchemaError(error) {
  return error instanceof Error
    ...
    && error.message.includes("laliga_matches");
}
...
async function rematchRows(config, rows, batchSize) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_laliga_match_id: matchExistingLaligaMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("laliga_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_laliga_match_id).length,
    laligaMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_laliga_match_id).length,
    laligaMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
