...
const FIXED_WIN_MODEL = "ligue1_fixed_win_percentage_single_v1";
const GOAL_SCORER_MODEL = "ligue1_goal_scorer_percentage_single_v1";
...
const LIGUE1_SINGLE_PREDICTION_COLUMNS = [
  "advertised_start_at",
  "away_team_name",
  "bucket_sample_size",
  "home_team_name",
...
function buildFixedWinPredictions({ matchesById, predictedAt, snapshots, sourceDate, teamRecords }) {
  ...
  for (const snapshot of getLatestSnapshots(snapshots)) {
    if (!snapshot.favourite_team_name || !snapshot.matched_ligue1_match_id) {
      continue;
    }

    const match = matchesById.get(snapshot.matched_ligue1_match_id);
    ...
    rows.push({
      ...
      matched_ligue1_match_id: snapshot.matched_ligue1_match_id,
    ...
    for (const team of teams) {
      ...
      for (const candidate of candidates) {
        ...
        rows.push({
          ...
          matched_ligue1_match_id: match.id,
...
function normalizeRowForWrite(row) {
  return Object.fromEntries(LIGUE1_SINGLE_PREDICTION_COLUMNS.map((column) => [
    column,
    row[column] ?? null,
  ]));
}
...
async function readSourceRows(supabase) {
  const [snapshots, matches, aggregates] = await Promise.all([
    supabase.selectAll("ligue1_market_snapshots", {
    ...
    supabase.selectAll("ligue1_insight_aggregates", {
...
async function writeRows(supabase, rows, sourceDate) {
  await supabase.request("ligue1_single_predictions", {
    expectJson: false,
    method: "DELETE",
  ...
  await supabase.upsert(
    "ligue1_single_predictions",
    rows.map(normalizeRowForWrite),
    "source_prediction_key",
  );

  return {
    ligue1SinglePredictions: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(sourceRows, fixedWinRows, goalScorerRows) {
  return {
    ...
    sourceLigue1InsightAggregates: sourceRows.aggregates.length,
    sourceLigue1Matches: sourceRows.matches.length,
