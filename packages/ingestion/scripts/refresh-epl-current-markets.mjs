...
/**
 * Parses the EPL current-market refresh orchestration options.
 */
...
/**
 * Builds the ordered EPL current-market refresh command list.
 */
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_epl_fixed_win_markets", "refresh-epl-market-snapshots-from-tab.mjs", [
  ...
  if (!options.skipGoalScorers || !options.skipReconcile || !options.skipPredictions) {
    commands.push(buildCommand("refresh_priced_epl_official_matches", "refresh-epl-results-from-official.mjs", [
      "--include-fixtures",
      "--priced-only",
      ...writeFlags,
    ]));
  ...
  if (!options.skipGoalScorers) {
    commands.push(buildCommand("capture_epl_goal_scorer_markets", "refresh-epl-goal-scorer-market-snapshots-from-tab.mjs", [
...
/**
 * Runs the EPL current-market capture and derived read-model refresh pipeline.
 */
async function main() {
