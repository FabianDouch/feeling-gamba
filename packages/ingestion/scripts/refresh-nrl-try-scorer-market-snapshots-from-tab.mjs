...
const NRL_TRY_SCORER_MARKET_NAME = "anytime try scorer";
const TAB_SOURCE = {
...
const NRL_TRY_SCORER_SNAPSHOT_QUERY = `
  query NrlTryScorerMarketSnapshot(
...
/**
 * Parses NRL player try-scorer snapshot ingestion options.
 */
...
/**
 * Builds browser-like headers for the current NRL market source request.
 */
...
function findTryScorerMarkets(event) {
  return (event.markets?.nodes ?? []).filter((market) =>
    normalizeName(market?.name) === NRL_TRY_SCORER_MARKET_NAME);
}
...
/**
 * Matches a current market event to one official NRL fixture shell/result.
 */
function matchExistingNrlMatch(eventContext, matches) {
...
/**
 * Matches a market player entrant to one official NRL match appearance.
 */
...
function mapTryScorerRows({ appearancesByMatch, event, eventContext, generatedAt, matchedMatch, source }) {
  ...
  for (const market of tryScorerMarkets) {
    ...
    for (const entrant of market.entrants?.nodes ?? []) {
      ...
      if (!parsed.playerName || entrant?.isSuspended === true) {
        continue;
      }
...
/**
 * Minimal Supabase REST client for NRL try-scorer snapshot reads and writes.
 */
...
/**
 * Fetches current open NRL events from the market source.
 */
async function fetchSourceEvents(options) {
  const response = await graphql(TAB_SOURCE, "NrlTryScorerMarketSnapshot", NRL_TRY_SCORER_SNAPSHOT_QUERY, {
...
/**
 * Fetches official NRL fixture rows in the current market event window.
 */
async function fetchOfficialMatchesForEvents(supabase, events) {
  ...
  return await supabase.selectAll("nrl_matches", {
...
async function fetchAppearancesForMatches(supabase, matches) {
  ...
  return await supabase.selectAll("nrl_player_match_appearances", {
...
function buildSnapshotRows(events, officialMatches, appearances) {
  ...
  for (const event of events) {
    ...
    const matchedMatch = matchExistingNrlMatch(eventContext, officialMatches);
    ...
    eventSummaries.push({
      eventName: event?.name ?? null,
      matchedNrlMatchId: matchedMatch?.id ?? null,
...
/**
 * Writes current NRL try-scorer price snapshots to Supabase.
 */
async function writeRows(supabase, rows) {
  await supabase.upsert("nrl_try_scorer_market_snapshots", rows, "source_selection_key");

  return {
    nrlTryScorerMarketSnapshots: rows.length,
    ok: true,
    skipped: false,
  };
}
...
/**
 * Summarizes current NRL try-scorer snapshot matching and price coverage.
 */
function summarize({ eventCount, officialMatches, result }) {
  return {
    generatedAt: result.generatedAt,
    matchedEvents: result.eventSummaries.filter((event) => event.matchedNrlMatchId).length,
    matchedPlayerSelections: result.rows.filter((row) => row.player_source_id).length,
    nrlTryScorerMarketSnapshots: result.rows.length,
    ...
    unmatchedEvents: result.eventSummaries.filter((event) =>
      !event.reason && !event.matchedNrlMatchId).length,
...
/**
 * Runs the local NRL player try-scorer market snapshot workflow.
 */
async function main() {
