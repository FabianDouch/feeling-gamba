...
const FIXED_WIN_MODEL = "laliga_fixed_win_percentage_single_v1";
const GOAL_SCORER_MODEL = "laliga_goal_scorer_percentage_single_v1";
...
const LALIGA_SINGLE_PREDICTION_COLUMNS = [
  "advertised_start_at",
  "away_team_name",
  "bucket_sample_size",
  "home_team_name",
...
function buildFixedWinPredictions({ matchesById, predictedAt, snapshots, sourceDate, teamRecords }) {
  ...
  for (const snapshot of getLatestSnapshots(snapshots)) {
    if (!snapshot.favourite_team_name || !snapshot.matched_laliga_match_id) {
      continue;
    }

    const match = matchesById.get(snapshot.matched_laliga_match_id);
    ...
    rows.push({
      ...
      matched_laliga_match_id: snapshot.matched_laliga_match_id,
    ...
    for (const team of teams) {
      ...
      for (const candidate of candidates) {
        ...
        rows.push({
          ...
          matched_laliga_match_id: match.id,
...
function normalizeRowForWrite(row) {
  return Object.fromEntries(LALIGA_SINGLE_PREDICTION_COLUMNS.map((column) => [
    column,
    row[column] ?? null,
  ]));
}
...
async function readSourceRows(supabase) {
  const [snapshots, matches, aggregates] = await Promise.all([
    supabase.selectAll("laliga_market_snapshots", {
    ...
    supabase.selectAll("laliga_insight_aggregates", {
...
async function writeRows(supabase, rows, sourceDate) {
  await supabase.request("laliga_single_predictions", {
    expectJson: false,
    method: "DELETE",
  ...
  await supabase.upsert(
    "laliga_single_predictions",
    rows.map(normalizeRowForWrite),
    "source_prediction_key",
  );

  return {
    laligaSinglePredictions: rows.length,
    ok: true,
    skipped: false,
  };
}
...
function summarize(sourceRows, fixedWinRows, goalScorerRows) {
  return {
    ...
    sourceLaligaInsightAggregates: sourceRows.aggregates.length,
    sourceLaligaMatches: sourceRows.matches.length,
