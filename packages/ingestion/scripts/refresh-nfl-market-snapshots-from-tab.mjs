...
const NFL_CATEGORY = "AMERICAN_FOOTBALL";
const NFL_COMPETITION_SLUG = "nfl";
const NFL_FIXED_WIN_MARKET_NAME = "head to head";
const TAB_SOURCE = {
  endpoint: "https://api.tab.co.nz/graphql",
  label: "NFL market source",
  source: "tab",
};

const NFL_COMPETITION_QUERY = `
...
/**
 * Parses the NFL fixed-win snapshot options for the current market source.
 */
...
/**
 * Builds browser-like headers for the NFL market source request.
 */
...
function findFixedWinMarket(event) {
  return (event.markets?.nodes ?? []).find((market) =>
    normalizeName(market?.name) === NFL_FIXED_WIN_MARKET_NAME);
}
...
function mapSnapshot(source, event, generatedAt, officialMatches) {
  ...
  const matchedMatch = matchExistingNflMatch(snapshot, officialMatches);

  return {
    ...snapshot,
    matched_nfl_match_id: matchedMatch?.id ?? null,
  };
}

/**
 * Fetches current NFL fixed-win snapshots from the selected public market source.
 */
async function fetchSnapshots(options, officialMatches) {
  ...
  for (const source of [TAB_SOURCE]) {
    const response = await graphql(source, "SportingCompetitionScreen", NFL_COMPETITION_QUERY, {
      category: NFL_CATEGORY,
      competitionSlug: NFL_COMPETITION_SLUG,
...
/**
 * Minimal Supabase REST client for NFL market snapshot reads and writes.
 */
...
async function fetchOfficialMatchesForWindow(config, rows, batchSize) {
  ...
  return await supabase.request("nfl_matches", {
    search: {
...
async function rematchRows(config, rows, batchSize) {
  ...
  try {
    officialMatches = await fetchOfficialMatchesForWindow(config, rows, batchSize);
  } catch (error) {
  ...
  return {
    matchedRows: rows.map((row) => ({
      ...row,
      matched_nfl_match_id: matchExistingNflMatch(row, officialMatches)?.id ?? null,
    })),
    officialMatches,
  };
}
...
async function writeRows(rows, options) {
  ...
  await supabase.upsert("nfl_market_snapshots", matchedRows, "source_snapshot_key");

  return {
    matchedSnapshots: matchedRows.filter((row) => row.matched_nfl_match_id).length,
    nflMarketSnapshots: matchedRows.length,
...
function summarize(snapshotResult, officialMatches) {
  return {
    ...
    matchedSnapshots: snapshotResult.rows.filter((row) => row.matched_nfl_match_id).length,
    nflMarketSnapshots: snapshotResult.rows.length,
    officialMatchesChecked: officialMatches.length,
  ...
  if (options.dryRun) {
    console.log(JSON.stringify({
      ...
      sample: matchedRows.slice(0, 5).map((row) => ({
        ...
        matchedNflMatchId: row.matched_nfl_match_id,
        source: row.source,
