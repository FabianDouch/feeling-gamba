...
const DEFAULT_COMPETITION_SLUG = "spanish-la-liga";

/**
 * Parses the LaLiga current-market refresh orchestration options.
 */
...
/**
 * Builds the ordered LaLiga current-market refresh command list.
 */
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_laliga_fixed_win_markets", "refresh-laliga-market-snapshots-from-tab.mjs", [
  ...
  if (!options.skipGoalScorers || !options.skipReconcile || !options.skipPredictions) {
    commands.push(buildCommand("refresh_priced_laliga_official_matches", "refresh-laliga-results-from-official.mjs", [
      "--include-fixtures",
      "--priced-only",
      ...writeFlags,
    ]));
  ...
  if (!options.skipGoalScorers) {
    commands.push(buildCommand("capture_laliga_goal_scorer_markets", "refresh-laliga-goal-scorer-market-snapshots-from-tab.mjs", [
      `--competition-slug=${options.competitionSlug}`,
  ...
  if (!options.skipReconcile) {
    commands.push(buildCommand("reconcile_laliga_fixed_win", "reconcile-laliga-fixed-win-snapshots.mjs", writeFlags));
  }

  if (!options.skipSameGameMultis) {
    commands.push(buildCommand("rebuild_laliga_same_game_multis", "rebuild-laliga-same-game-multis.mjs", writeFlags));
  }

  if (!options.skipInsights) {
    commands.push(buildCommand("rebuild_laliga_insights", "rebuild-laliga-insight-aggregates.mjs", writeFlags));
  }

  if (!options.skipPredictions) {
    commands.push(buildCommand("generate_laliga_single_predictions", "generate-laliga-single-predictions.mjs", writeFlags));
  }
...
/**
 * Runs the LaLiga current-market capture and derived read-model refresh pipeline.
 */
async function main() {
