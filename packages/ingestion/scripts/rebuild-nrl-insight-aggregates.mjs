...
function buildFixedWinAggregates(results, matchesById) {
  ...
  for (const record of allRecords) {
    ...
    addToBucket(buckets, {
      ...
      scopeKey: `nrl:fixed_win_single:price_bucket:${record.selectionType}:${priceBucket.start.toFixed(2)}`,
  ...
  for (const record of allRecords) {
    ...
    if (otherPriceBucket) {
      addToBucket(buckets, {
        ...
        scopeKey: `nrl:fixed_win_single:other_team_price_bucket:${record.selectionType}:${otherPriceBucket.start.toFixed(2)}`,
    ...
    if (differenceBucket) {
      addToBucket(buckets, {
        ...
        scopeKey: `nrl:fixed_win_single:price_difference_bucket:${record.selectionType}:${differenceBucket.start.toFixed(2)}`,
