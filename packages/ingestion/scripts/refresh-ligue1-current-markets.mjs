...
const DEFAULT_COMPETITION_SLUG = "french-ligue-1";

/**
 * Parses the Ligue 1 current-market refresh orchestration options.
 */
...
/**
 * Builds the shared Supabase/write flags used by the child Ligue 1 workers.
 */
...
/**
 * Builds the ordered Ligue 1 current-market refresh command list.
 */
function buildRefreshCommands(options) {
  ...
  if (!options.skipFixedWin) {
    commands.push(buildCommand("capture_ligue1_fixed_win_markets", "refresh-ligue1-market-snapshots-from-tab.mjs", [
  ...
  if (!options.skipGoalScorers || !options.skipReconcile || !options.skipPredictions) {
    commands.push(buildCommand("refresh_priced_ligue1_official_matches", "refresh-football-league-results-from-json.mjs", [
      "--league=ligue1",
      "--include-fixtures",
      "--priced-only",
      ...writeFlags,
  ...
  if (!options.skipGoalScorers) {
    commands.push(buildCommand("capture_ligue1_goal_scorer_markets", "refresh-ligue1-goal-scorer-market-snapshots-from-tab.mjs", [
      `--competition-slug=${options.competitionSlug}`,
  ...
  if (!options.skipReconcile) {
    commands.push(buildCommand("reconcile_ligue1_fixed_win", "reconcile-ligue1-fixed-win-snapshots.mjs", writeFlags));
  }

  if (!options.skipSameGameMultis) {
    commands.push(buildCommand("rebuild_ligue1_same_game_multis", "rebuild-ligue1-same-game-multis.mjs", writeFlags));
  }

  if (!options.skipInsights) {
    commands.push(buildCommand("rebuild_ligue1_insights", "rebuild-ligue1-insight-aggregates.mjs", writeFlags));
  }

  if (!options.skipPredictions) {
    commands.push(buildCommand("generate_ligue1_single_predictions", "generate-ligue1-single-predictions.mjs", writeFlags));
  }
...
/**
 * Runs the Ligue 1 current-market capture and derived read-model refresh pipeline.
 */
async function main() {
